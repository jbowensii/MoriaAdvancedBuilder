...
UObject* getCachedBuildHUD()
{
    ...
    if (hud && safeClassName(hud).find(L"BuildHUD") != std::wstring::npos) return hud;
...
void showBuildTab()
{
    ...
    if (!tab)
    {
        QBLOG(STR("[MoriaCppMod] [QB] showBuildTab: no tab found\n"));
        return;
    }
    auto* fn = tab->GetFunctionByNameInChain(STR("Show"));
    QBLOG(STR("[MoriaCppMod] [QB] showBuildTab: calling Show() fn={}\n"), fn ? STR("YES") : STR("NO"));
    if (!safeProcessEvent(tab, fn, nullptr))
    {
        QBLOG(STR("[MoriaCppMod] [QB] showBuildTab: safeProcessEvent FAILED\n"));
        return;
    }
...
void hideBuildTab()
{
    ...
    if (!tab)
    {
        QBLOG(STR("[MoriaCppMod] [QB] hideBuildTab: no tab found\n"));
        return;
    }
    ...
    if (!safeProcessEvent(tab, fn, nullptr))
    {
        QBLOG(STR("[MoriaCppMod] [QB] hideBuildTab: safeProcessEvent FAILED\n"));
        return;
    }
    m_lastShowHideTime = now;
}
...
struct OriginalRelLoc
{
    float x, y, z;
};
struct OriginalRelRot
{
...
bool resolveGATAOffsets(UObject* gata)
{
    ...
    if (!p1 || !p2)
    {
        VLOG(STR("[MoriaCppMod] [PitchRoll] Failed to resolve TraceResults offsets\n"));
        return false;
    }
    ...
    if (!structProp)
    {
        VLOG(STR("[MoriaCppMod] [PitchRoll] TraceResults is not FStructProperty\n"));
        return false;
    }
    ...
    if (innerStruct)
    {
        ...
        {
            if (sp->GetName() == STR("TargetRotation"))
            {
                m_offTargetRotation = sp->GetOffset_Internal();
                break;
            }
        }
    }
    if (m_offTargetRotation < 0)
    {
        VLOG(STR("[MoriaCppMod] [PitchRoll] Failed to resolve TargetRotation offset\n"));
        return false;
    }

    VLOG(STR("[MoriaCppMod] [PitchRoll] Resolved: TraceResults=+0x{:04X} LastTrace=+0x{:04X} TargetRot=+0x{:04X}\n"),
         m_offTraceResults,
         m_offLastTraceResults,
         m_offTargetRotation);
    return true;
}
...
bool resolveReticleOffsets(UObject* reticle)
{
    ...
    if (m_offCopiedComponents < 0 || m_offRelativeRotation < 0 || m_offRelativeLocation < 0)
    {
        VLOG(STR("[MoriaCppMod] [PitchRoll] Failed to resolve reticle offsets (CC={} RR={} RL={})\n"),
             m_offCopiedComponents,
             m_offRelativeRotation,
             m_offRelativeLocation);
        return false;
    }
...
void injectPitchRoll(float pitch, float roll)
{
    ...
    showOnScreen(std::format(L"Pitch={:.0f}\u00B0 Roll={:.0f}\u00B0", pitch, roll), 3.0f, 1.0f, 0.8f, 0.0f);
}
...
static void quatRotVec(const Quat4& q, float vx, float vy, float vz, float& ox, float& oy, float& oz)
{
...
static EulerAngles quatToEuler(const Quat4& q)
{
    ...
    float pitch = (fabsf(singularityTest) >= 0.4999995f) ? copysignf(90.0f, singularityTest) : asinf(2.0f * singularityTest) * R2D;
...
void rotateReticleVisual(UObject* gata, float pitchDeg, float rollDeg)
{
    ...
    if (!rootProp) return;
    UObject* rootComp = *reinterpret_cast<UObject**>(reinterpret_cast<uint8_t*>(reticle) + rootProp->GetOffset_Internal());
    ...
    if (!m_origRelLocsCached)
    {
        ...
        for (int ci = 0; ci < arr->count; ci++)
        {
            ...
            {
                m_origRelLocs[ci] = {0, 0, 0};
                m_origRelRots[ci] = {0, 0, 0};
                continue;
            }
        ...
        if (foundSnap) VLOG(STR("[MoriaCppMod] [PitchRoll] Pivot: ({:.1f},{:.1f},{:.1f})\n"), m_pivotPoint.x, m_pivotPoint.y, m_pivotPoint.z);
    ...
    for (int i = 0; i < arr->count; i++)
    {
        ...
        if (!compObj->GetFunctionByNameInChain(STR("GetLocalBounds"))) continue;
        ...
        if (setFn)
        {
            ...
            for (auto* prop : setFn->ForEachProperty())
            {
                ...
                if (pname == STR("NewLocation"))
                    offLoc = prop->GetOffset_Internal();
                else if (pname == STR("NewRotation"))
                    offRot = prop->GetOffset_Internal();
                else if (pname == STR("bTeleport"))
                    offTeleport = prop->GetOffset_Internal();
            }
            ...
            if (offLoc >= 0 && offRot >= 0)
            {
                ...
                rot[0] = finalRot.pitch;
                rot[1] = finalRot.yaw;
                rot[2] = finalRot.roll;

                if (offTeleport >= 0) *reinterpret_cast<bool*>(params.data() + offTeleport) = true;
...
void tickPitchRoll()
{
    ...
    if (!m_pitchRotateEnabled && !m_rollRotateEnabled)
    {
...
void onGhostDisappeared()
{
    ...
    ULONGLONG now = GetTickCount64();
    if (now - m_lastQBSelectTime < 500) return;
    VLOG(STR("[MoriaCppMod] [Placement] onGhostDisappeared: greying out snap slot\n"));
}

// Read a build-item widget's own bLock via the property chain — per-class-
// correct offset regardless of which UI_WBP_Build_Item_* subclass it is.
uint8_t* widgetBLockPtr(UObject* w)
{
    if (!w || !isObjectAlive(w)) return nullptr;
    uint8_t* p = w->GetValuePtrByPropertyNameInChain<uint8_t>(STR("bLock"));
    if (!p || !isReadableMemory(p, BLOCK_DATA_SIZE)) return nullptr;
    return p;
}

// True when the widget's bLock carries a populated Variants array. Pooled/
// recycled menu tiles can pass a name match while holding an empty block,
// and blockSelectedEvent with an empty block selects nothing (no ghost).
bool widgetBLockHasVariants(UObject* w)
{
    uint8_t* p = widgetBLockPtr(w);
    if (!p) return false;
    int32_t num = *reinterpret_cast<int32_t*>(p + rbVariantsNumOff());
...
SelectResult selectRecipeOnBuildTab(UObject* buildTab, int slot)
{
    ...
    for (auto* w : widgets)
    {
        ...
        if (!name.empty() && name == targetName)
        {
            ...
            // Menu tiles are pooled/recycled: a name-matching instance can
            // carry an EMPTY bLock (Variants=0), and blockSelectedEvent with
            // an empty block selects nothing — the "toast shows but no ghost
            // spawns" failure. Remember the first name-match as a fallback,
            // but keep scanning for one whose block actually has variants.
            if (!matchedWidget) matchedWidget = w;
            if (widgetBLockHasVariants(w))
            {
                matchedWidget = w;
                break;
            }
    ...
    // Fresh bLock is only trusted when its Variants array is populated.
    // A recycled tile can hold an EMPTY block; blockSelectedEvent with an
    // empty block selects nothing — the game shows no ghost while our
    // toast claims success (2026-07-13 root cause of "F-key does nothing
    // after assign"). The property is read via the class chain so the
    // offset is always correct for the widget's actual class.
    if (uint8_t* blk = widgetBLockPtr(matchedWidget))
    {
        if (widgetBLockHasVariants(matchedWidget))
        {
            std::memcpy(params.data() + s_bse.bLock, blk, BLOCK_DATA_SIZE);
            gotFreshBLock = true;
        }
        else
        {
            VLOG(STR("[MoriaCppMod] [QuickBuild] fresh bLock on matched tile has Variants=0 — using saved capture\n"));
        }
    }
    ...
    if (!gotFreshBLock && m_recipeSlots[slot].hasBLockData)
    {
        ...
        VLOG(STR("[MoriaCppMod] [QuickBuild] using SAVED capture bLock for '{}'\n"), targetName);
    }
    else if (!gotFreshBLock)
    {
        VLOG(STR("[MoriaCppMod] [QuickBuild] WARNING: no bLock data at all for '{}' — selection will likely no-op\n"), targetName);
    }
    ...
    *reinterpret_cast<int32_t*>(params.data() + s_bse.Index) = 0;
    ...
    if (!safeProcessEvent(buildTab, func, params.data()))
    {
        QBLOG(STR("[MoriaCppMod] [QB] blockSelectedEvent safeProcessEvent FAILED\n"));
        return SelectResult::NotFound;
    }
    m_isAutoSelecting = false;

    if (buildHUD) cacheRecipeHandleForSlot(buildHUD, slot);
...
SelectResult selectRecipeByTargetName(UObject* buildTab)
{
    ...
    if (uint8_t* blk = widgetBLockPtr(matchedWidget))
    {
        ...
        gotFreshBLock = true;
        if (!widgetBLockHasVariants(matchedWidget))
            VLOG(STR("[MoriaCppMod] [TargetBuild] WARNING: matched tile's bLock has Variants=0 — selection may no-op\n"));
    }
...
void startOrSwitchBuild(int slot)
{
    ...
    if (!m_recipeSlots[slot].rowName.empty())
    {
        ...
        if (buildHUD && haveHandle)
        {
            ...
            {
                m_isAutoSelecting = true;
                struct AutoSelectGuardDirect
                {
                    bool& f;
                    ~AutoSelectGuardDirect()
                    {
                        f = false;
                    }
                } guardDirect{m_isAutoSelecting};
...
void placementTick()
{
    ...
    if (m_qbPhase == PlacePhase::SelectRecipeWalk)
    {
        ...
        SelectResult result = m_isTargetBuild ? selectRecipeByTargetName(buildTab) : selectRecipeOnBuildTab(buildTab, m_pendingQuickBuildSlot);
