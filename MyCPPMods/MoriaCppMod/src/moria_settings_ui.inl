...
// auto-recover from missed Hide hook. If flag is set but the
...
UClass* m_settingsCheckBoxCls{nullptr}; // for ON/OFF toggles
UClass* m_settingsCarouselCls{nullptr}; // for cycle rows (tweaks)
// captured FSlateBrush bytes from a native CheckIcon
...
void onSettingsRelatedShown(UObject* widget, const wchar_t* fnName)
{
    ...
    try
    {
        if (UClass* uc = static_cast<UClass*>(widget->GetClassPrivate())) path = uc->GetFullName();
    }
    catch (...)
    {
    }
...
void tickSettingsUI()
{
    ...
    if (editMap && isObjectAlive(editMap))
    {
        ...
        for (const wchar_t* probeName : {STR("AttackMineKey"), STR("Build"), STR("CalloutKey"), STR("CrouchKey"), STR("DodgeKey"), STR("ChatKey")})
        {
            ...
            try
            {
                outer = selWidget->GetOuterPrivate();
            }
            catch (...)
            {
            }
...
std::vector<ModKeymapRow> buildInventoryRows()
{
    ...
    static const wchar_t* useLabels[8] = {STR("Quick Build 1"),
                                          STR("Quick Build 2"),
                                          STR("Quick Build 3"),
                                          STR("Quick Build 4"),
                                          STR("Quick Build 5"),
                                          ...
                                          STR("Quick Build 7"),
                                          STR("Quick Build 8")};
    static const wchar_t* setLabels[8] = {STR("Quick Build 1 Set"),
                                          STR("Quick Build 2 Set"),
                                          STR("Quick Build 3 Set"),
                                          STR("Quick Build 4 Set"),
                                          STR("Quick Build 5 Set"),
                                          STR("Quick Build 6 Set"),
                                          STR("Quick Build 7 Set"),
                                          STR("Quick Build 8 Set")};
    static const wchar_t* useIniKeys[8] = {STR("QuickBuild1.Use"),
                                           STR("QuickBuild2.Use"),
                                           STR("QuickBuild3.Use"),
                                           STR("QuickBuild4.Use"),
                                           STR("QuickBuild5.Use"),
                                           ...
                                           STR("QuickBuild7.Use"),
                                           STR("QuickBuild8.Use")};
    static const wchar_t* setIniKeys[8] = {STR("QuickBuild1.Set"),
                                           STR("QuickBuild2.Set"),
                                           STR("QuickBuild3.Set"),
                                           STR("QuickBuild4.Set"),
                                           STR("QuickBuild5.Set"),
...
bool ensureSettingsKeyClassesCached(UObject* editMapTab)
{
    ...
    {
        for (const wchar_t* probe : {STR("AttackMineKey"), STR("Build"), STR("ChatKey"), STR("CalloutKey"), STR("CrouchKey"), STR("DodgeKey")})
        {
    ...
    {
        for (const wchar_t* probe : {STR("ConstructionHeading"), STR("EmotesHeading"), STR("EquipmentHeading"), STR("GeneralHeading"), STR("MovementHeading")})
        {
            auto* p = editMapTab->GetValuePtrByPropertyNameInChain<UObject*>(probe);
            UObject* w = p ? *p : nullptr;
            if (w && isObjectAlive(w))
                         ...
                         std::vector<UObject*>& outSelectorsToBind)
{
    ...
    if (auto* namePtr = selector->GetValuePtrByPropertyNameInChain<FText>(STR("OptionName"))) *namePtr = FText(labelText);
...
std::wstring vkToFKeyName(uint8_t vk)
{
    ...
    case VK_NUMPAD6:
        return L"NumPadSix";
    case VK_NUMPAD7:
        return L"NumPadSeven";
    case VK_NUMPAD8:
        return L"NumPadEight";
    case VK_NUMPAD9:
        return L"NumPadNine";
    case VK_MULTIPLY:
        return L"Multiply";
    ...
    if (vk >= '0' && vk <= '9')
    {
        static const wchar_t* digitNames[] = {L"Zero", L"One", L"Two", L"Three", L"Four", L"Five", L"Six", L"Seven", L"Eight", L"Nine"};
...
void applyDefaultChordToSelector(UObject* selector, uint8_t vk, bool withShift)
{
    ...
    if (auto* dst = selector->GetValuePtrByPropertyNameInChain<uint8_t>(STR("CurrentSelectedKey"))) std::memcpy(dst, chord, 0x20);
    if (auto* innerPtr = selector->GetValuePtrByPropertyNameInChain<UObject*>(STR("OptionKeySelector")))
    {
        if (UObject* inner = *innerPtr)
        {
            if (auto* sk = inner->GetValuePtrByPropertyNameInChain<uint8_t>(STR("SelectedKey"))) std::memcpy(sk, chord, 0x20);
        }
    }
    ...
    if (auto* innerPtr = selector->GetValuePtrByPropertyNameInChain<UObject*>(STR("OptionKeySelector")))
    {
        if (UObject* inner = *innerPtr)
        {
            if (auto* fn = inner->GetFunctionByNameInChain(STR("SetAllowModifierKeys")))
            {
                ...
                if (p)
                {
                    b[p->GetOffset_Internal()] = 1;
                    safeProcessEvent(inner, fn, b.data());
                }
            }
            if (auto* esc = inner->GetValuePtrByPropertyNameInChain<TArray<uint8_t>>(STR("EscapeKeys"))) esc->SetNum(0);
        }
    }
...
void injectModKeybindRows(UObject* editMapTab)
{
    ...
    auto trackRow = [this](UObject* selector, int bindIdx, uint8_t vk, bool isSetRow, uint8_t modBits, const char* iniKey) {
        if (!selector) return;
        KeymapRow r;
    ...
    if (UObject* hd = spawnSectionHeading(STR("Quick Build Menu"))) addToScrollBox(scrollBox, hd);
    int invAdded = 0;
    for (int i = 0; i < 8; ++i)
    {
        wchar_t buf[32];
        ...
        UObject* useRow = spawnKeybindRow(outer, buf, nullptr, s_bindings[i].key, false, selectors);
        if (useRow)
        {
        ...
        UObject* setRow = spawnKeybindRow(outer, buf, nullptr, s_setBindings[i].vk, (s_setBindings[i].modBits & 0x01) != 0, selectors);
        if (setRow)
        {
    ...
    if (UObject* hd = spawnSectionHeading(STR("Mod Key Bindings"))) addToScrollBox(scrollBox, hd);
    int modAdded = 0;
    ...
    // Snap Toggle added under Building.
...
// Insert a row into the keymap ScrollBox right before the
...
bool insertIntoNativeKeymapSection(UObject* scrollBox, UObject* editMapTab, const wchar_t* headingMemberName, UObject* newRow)
{
    ...
    if (auto* gccFn = scrollBox->GetFunctionByNameInChain(STR("GetChildrenCount")))
    {
        ...
        if (auto* rp = findParam(gccFn, STR("ReturnValue"))) childCount = *reinterpret_cast<int32_t*>(b.data() + rp->GetOffset_Internal());
    }
    ...
    auto getChildAt = [&](int idx) -> UObject* {
        ...
        std::vector<uint8_t> b(sz, 0);
        if (auto* p = findParam(fn, STR("Index"))) *reinterpret_cast<int32_t*>(b.data() + p->GetOffset_Internal()) = idx;
        safeProcessEvent(scrollBox, fn, b.data());
        if (auto* rp = findParam(fn, STR("ReturnValue"))) return *reinterpret_cast<UObject**>(b.data() + rp->GetOffset_Internal());
        return nullptr;
    };
...
UObject* tryLoadGlyph(const wchar_t* keyName)
{
    ...
    for (auto* prefix : prefixes)
    {
        for (const auto& name : nameVariants)
        {
            ...
            try
            {
                tex = UObjectGlobals::StaticFindObject<UObject*>(nullptr, nullptr, p.c_str());
            }
            catch (...)
            {
            }
...
void tickCaptureSpecialKeys()
{
    ...
    for (auto& r : m_keymapRows)
    {
        ...
        auto applyCapture = [&](uint8_t vk, const std::wstring& fname) {
            ...
            if (auto* dst = sel->GetValuePtrByPropertyNameInChain<uint8_t>(STR("CurrentSelectedKey"))) std::memcpy(dst, chord, 0x20);
            if (auto* sk = inner->GetValuePtrByPropertyNameInChain<uint8_t>(STR("SelectedKey"))) std::memcpy(sk, chord, 0x20);
            ...
            if (auto* selecting = inner->GetValuePtrByPropertyNameInChain<uint8_t>(STR("bIsSelectingKey"))) *selecting = 0;
...
void tickReapplyModifierPrefixes()
{
    ...
    if (!m_glyphTex_Shift && !m_glyphTex_Ctrl && !m_glyphTex_Alt) return; // textures missing — nothing to render
    ...
    for (auto& r : m_keymapRows)
    {
        ...
        if (!glyph)
        {
            ...
            if (auto* p = sel->GetValuePtrByPropertyNameInChain<UObject*>(STR("KeyGlyph"))) nativeKeyGlyph = *p;
            ...
            if (nativeScaleBox)
            {
                if (auto* sp = nativeScaleBox->GetValuePtrByPropertyNameInChain<uint8_t>(STR("Stretch"))) nativeStretch = *sp;
            }
            if (auto* fn = sbox->GetFunctionByNameInChain(STR("SetStretch")))
            {
                ...
                if (p)
                {
                    b[p->GetOffset_Internal()] = nativeStretch;
        ...
        if (needGlyph && tex && m_imageSetBrushFn)
        {
            ...
            auto callIntReturn = [&](const wchar_t* fname) -> int32_t {
                ...
                if (p && p->GetOffset_Internal() + (int)sizeof(int32_t) <= sz) return *reinterpret_cast<int32_t*>(b.data() + p->GetOffset_Internal());
                return 0;
            };
...
void onModSelectorRebound(UObject* selector)
{
    ...
    for (auto& r : m_keymapRows)
    {
        ...
        VLOG(STR("[SettingsUI] rebind '{}' -> Key='{}' Shift={}\n"), utf8ToWide(r.iniKey).c_str(), keyStr.c_str(), isShift ? STR("Y") : STR("N"));
        ...
        bool isModifierAlone = keyStr == L"LeftShift" || keyStr == L"RightShift" || keyStr == L"Shift" || keyStr == L"LeftControl" ||
                               keyStr == L"RightControl" || keyStr == L"Control" || keyStr == L"LeftAlt" || keyStr == L"RightAlt" || keyStr == L"Alt" ||
        ...
        if (isModifierAlone)
        {
            ...
            // show "Shift" alone after rejection.
            applyDefaultChordToSelector(selector, r.vk, (r.modBits & 0x01) != 0);
            if (auto* innerPtr = selector->GetValuePtrByPropertyNameInChain<UObject*>(STR("OptionKeySelector")))
            {
                if (UObject* inner = *innerPtr)
                {
                    if (auto* sel2 = inner->GetValuePtrByPropertyNameInChain<uint8_t>(STR("bIsSelectingKey"))) *sel2 = 1;
                }
            }
            return;
        }
...
uint8_t fkeyNameToVK(const std::wstring& fname)
{
    ...
    if (fname.size() >= 2 && fname[0] == L'F')
    {
        ...
        if (n >= 1 && n <= 12 && end && *end == 0) return static_cast<uint8_t>(0x70 + n - 1);
    }
    ...
    if (fname.size() == 1 && fname[0] >= L'A' && fname[0] <= L'Z') return static_cast<uint8_t>(fname[0]);
    return 0;
}
...
struct GameOptButton
{
    ...
    GameOptKind kind;
    bool fromPauseMenu{false}; // protect from Gameplay-tab clear
};
...
UObject* spawnGameOptButton(UObject* outer, const wchar_t* labelText, GameOptKind kind, bool fromPauseMenu = false)
{
    ...
    if (auto* lblPtr = btn->GetValuePtrByPropertyNameInChain<FText>(STR("ButtonLabel"))) *lblPtr = FText(labelText);
...
void captureGameplayNativeChildren(UObject* parentPanel)
{
    ...
    for (int i = 0; i < slots->Num(); ++i)
    {
        ...
        VLOG(
                STR("[SettingsUI] CP5-DBG —   child[{}]: cls={} name={} origVis={}\n"),
                i,
                safeClassName(child).c_str(),
                [&] {
                    try
                    {
...
void injectModGameOptions(UObject* gameplayTab)
{
    ...
    if (isCheatsHost)
    {
        ...
        if (m_settingsSectionHeadingCls)
        {
            if (UObject* hd = spawnSectionHeading(STR("Cheats"))) appendToVerticalBox(parentPanel, hd);
        }
        ...
        for (const auto& s : cheatSpecs)
        {
            UObject* btn = jw_createGameWidget(m_frontEndButtonCls);
            if (!btn) continue;
            if (auto* lblPtr = btn->GetValuePtrByPropertyNameInChain<FText>(STR("ButtonLabel"))) *lblPtr = FText(s.label);
            if (auto* fn = btn->GetFunctionByNameInChain(STR("UpdateTextLabel")))
            {
                std::vector<uint8_t> b(fn->GetParmsSize(), 0);
                auto* p = findParam(fn, STR("Text"));
            ...
            CheatButton entry;
            entry.widget = FWeakObjectPtr(btn);
            entry.kind = s.kind;
    ...
    // preserve pause-menu entries (same reason as Gameplay-Cheats path).
    std::erase_if(m_gameOptButtons, [](const GameOptButton& g) {
        return !g.fromPauseMenu;
    });

    if (m_settingsSectionHeadingCls)
    {
        if (UObject* hd = spawnSectionHeading(STR("Mod Game Options"))) appendToVerticalBox(parentPanel, hd);
    }
...
// Insert child into a UVerticalBox at a specific index.
...
// find the slot index of a named child widget within a
...
// Inject the 5 mod action buttons into the pause menu
...
void injectPauseMenuButtons(UObject* escapeMenu)
{
    ...
    if (!ensureFrontEndButtonClassCached(escapeMenu))
    {
        ...
        if (rb && isObjectAlive(rb)) m_frontEndButtonCls = static_cast<UClass*>(rb->GetClassPrivate());
    ...
    // ALL CAPS labels for uniformity with native buttons
    ...
    for (UObject* m : modButtons)
        finalOrder.push_back(m);
    ...
    try
    {
        spacerCls = UObjectGlobals::StaticFindObject<UClass*>(nullptr, nullptr, STR("/Script/UMG.SizeBox"));
    }
    catch (...)
    {
    }
    ...
    auto spawnSpacer = [&](float pixels) -> UObject* {
        ...
        try
        {
            sp = UObjectGlobals::StaticConstructObject(spP);
        }
        catch (...)
        {
        }
...
UObject* spawnBuffToggleRow(int cheatEntryIdx)
{
    ...
    {
        UObject* row = spawnCheckBoxRow(
                e.label,
                [this, cheatEntryIdx]() -> bool {
                    return (cheatEntryIdx >= 0 && cheatEntryIdx < (int)m_buffStates.size()) ? m_buffStates[cheatEntryIdx] : false;
                },
    ...
    if (auto* namePtr = row->GetValuePtrByPropertyNameInChain<FText>(STR("OptionName"))) *namePtr = FText(e.label);
    ...
    BuffToggleRow btr;
    btr.selector = FWeakObjectPtr(row);
    btr.cheatEntryIdx = cheatEntryIdx;
    m_buffToggleRows.push_back(btr);
    registerRowStatus(row, L"OFF", 0.65f, 0.65f, 0.65f, [this, cheatEntryIdx](std::wstring& t, float& r, float& g, float& b) {
        bool on = (cheatEntryIdx >= 0 && cheatEntryIdx < (int)m_buffStates.size()) ? m_buffStates[cheatEntryIdx] : false;
        if (on)
        {
            t = L"ON";
            r = 0.31f;
...
// Spawn a row for a tweak cycle. Status text shows the
...
UObject* spawnTweakCycleRow(int tweakIdx)
{
    ...
    UObject* row = jw_createGameWidget(m_settingsKeySelectorCls);
    if (!row) return nullptr;
    if (auto* namePtr = row->GetValuePtrByPropertyNameInChain<FText>(STR("OptionName"))) *namePtr = FText(t.label);
    if (auto* tbPtr = row->GetValuePtrByPropertyNameInChain<UObject*>(STR("OptionNameTextBlock")))
        if (UObject* tb = *tbPtr) umgSetText(tb, t.label);
    TweakCycleRow tcr;
    tcr.selector = FWeakObjectPtr(row);
    tcr.tweakIdx = tweakIdx;
    m_tweakCycleRows.push_back(tcr);
    // Dynamic value-label provider.
    registerRowStatus(row, L"DEFAULT", 0.65f, 0.65f, 0.65f, [this, tweakIdx](std::wstring& outT, float& r, float& g, float& b) {
        int nT2 = 0;
        ...
        if (curIdx >= (int)te.cycleValues.size())
        {
            outT = L"?";
            return;
        }
        ...
        wchar_t buf[32];
        if (te.kind == TweakKind::SpecialNoCost || te.kind == TweakKind::SpecialInstantCraft)
        {
            outT = (v == 0) ? L"OFF" : L"ON";
            if (v != 0)
            {
...
// Cache the WBP_SettingsCheckBox_C UClass by probing the
...
bool ensureSettingsCheckBoxClassCached(UObject* gameplayTabAny)
{
    ...
    if (cls != STR("WBP_GameplayTab_C")) gp = settingsScreenForTab(gameplayTabAny);
    if (gp)
    {
    ...
    for (const wchar_t* probe : {STR("MinimapLock"), STR("StreamerMode")})
    {
        ...
        if (w && isObjectAlive(w))
        {
            if (!m_settingsCheckBoxCls) m_settingsCheckBoxCls = static_cast<UClass*>(w->GetClassPrivate());
            // also capture the CheckIcon brush bytes so
...
// CheckBox toggle row tracking. Each entry maps the
...
UObject* spawnCheckBoxRow(const wchar_t* label, std::function<bool()> getCurrent, std::function<void(bool)> onChanged)
{
    if (m_settingsCarouselCls)
    {
        ...
        if (car)
        {
            if (auto* namePtr = car->GetValuePtrByPropertyNameInChain<FText>(STR("OptionName"))) *namePtr = FText(label);
    ...
    if (!row) return nullptr;
    if (auto* namePtr = row->GetValuePtrByPropertyNameInChain<FText>(STR("OptionName"))) *namePtr = FText(label);
    if (auto* tbPtr = row->GetValuePtrByPropertyNameInChain<UObject*>(STR("OptionNameTextBlock")))
        if (UObject* tb = *tbPtr) umgSetText(tb, label);
            ...
            if (ct && isObjectAlive(ct))
            {
                ...
                if (vib && isObjectAlive(vib))
                {
                    auto dumpCheckBox = [this](const wchar_t* tag, UObject* cb) {
                        ...
                        VLOG(STR("[SettingsUI] CP5-CMP {}: cb={:p} cls={}, OptionCheckBox={:p}, CheckIcon={:p} cls={}, iconVis={}, brush[0..3]={},{},{},{}, "
                                 "ResourceObject@brush+16={:p}\n"),
                             tag,
                             (void*)cb,
                             safeClassName(cb).c_str(),
                             (void*)inner,
                             (void*)icon,
                             icon ? safeClassName(icon).c_str() : L"null",
...
bool maybeFireCheckBoxRow(UObject* widget, bool newState)
{
    ...
    for (auto& cbr : m_checkBoxRows)
    {
        ...
        // swallow the delegate when the change came from
...
void refreshCheckBoxIcons()
{
    for (size_t idx = 0; idx < m_checkBoxRows.size(); ++idx)
    {
        ...
        // DO NOT call SetIsChecked here. It fires the
        ...
        if (auto* iconPtr = w->GetValuePtrByPropertyNameInChain<UObject*>(STR("CheckIcon")))
        {
            if (UObject* icon = *iconPtr) setWidgetVisibility(icon, on ? 0 : 1);
        }
    }
}

// Carousel row tracking + helpers. Each row maps to a
...
bool ensureSettingsCarouselClassCached(UObject* anyTab)
{
    ...
    UObject* gp = anyTab;
    std::wstring c = safeClassName(gp);
    if (c != STR("WBP_GameplayTab_C")) gp = settingsScreenForTab(anyTab);
...
UObject* spawnCarouselRow(int tweakIdx, const wchar_t* label, const std::vector<std::pair<std::wstring, std::wstring>>& options, const std::wstring& currentValue)
{
    ...
    // also write OptionText directly so the displayed
    ...
    try
    {
        curIdxInt = std::stoi(currentValue);
    }
    catch (...)
    {
    }
    ...
    CarouselRow cr;
    cr.widget = FWeakObjectPtr(row);
    cr.tweakIdx = tweakIdx;
...
bool maybeFireCarouselRow(UObject* widget, const std::wstring& newValue)
{
    for (auto& cr : m_carouselRows)
    {
        ...
        if ((int)m_tweakCurrentIdx.size() != nT) m_tweakCurrentIdx.assign(nT, 0);
...
bool maybeFireCarouselViaDelegate(UObject* widget, const wchar_t* fnName)
{
    ...
    for (auto& cr : m_carouselRows)
    {
        ...
        for (int h = 0; h < hops; ++h)
            cycleTweakValue(cr.tweakIdx);
        int curIdx = (cr.tweakIdx < (int)m_tweakCurrentIdx.size()) ? m_tweakCurrentIdx[cr.tweakIdx] : 0;
        if (auto* otPtr = widget->GetValuePtrByPropertyNameInChain<UObject*>(STR("OptionText")))
        {
            if (UObject* ot = *otPtr)
            {
...
// fallback for when the BP carousel doesn't fire its
...
bool maybeFireCarouselButton(UObject* clickedBtn)
{
    ...
    int hops = isNext ? 1 : (cycleLen - 1);
    for (int h = 0; h < hops; ++h)
        cycleTweakValue(matchedTweakIdx);

    // Push new value back to the carousel so its visual updates.
    int curIdx = (matchedTweakIdx < (int)m_tweakCurrentIdx.size()) ? m_tweakCurrentIdx[matchedTweakIdx] : 0;
...
// Build a label-left + button-right row for action items
...
UObject* spawnActionButtonRow(UObject* outer, const wchar_t* label, const wchar_t* buttonText, GameOptKind kind)
{
    if (!m_frontEndButtonCls) return nullptr;
    ...
    // two equal-width columns. Each column fills 50% of
    ...
    if (!btn) return row;
    if (UObject* innerTb = jw_findChildInTree(btn, STR("ButtonText"))) umgSetFontSize(innerTb, 18);

    // narrow SizeBox so the button is smaller than the
...
UObject* spawnModPackRow(int modIdx, const std::wstring& displayName, bool /*enabled*/)
{
    if (m_settingsCheckBoxCls)
    {
        return spawnCheckBoxRow(
                displayName.c_str(),
                [this, modIdx]() -> bool {
                    return (modIdx >= 0 && modIdx < (int)m_ftGameModEntries.size()) ? m_ftGameModEntries[modIdx].enabled : false;
                },
    ...
    UObject* row = jw_createGameWidget(m_settingsKeySelectorCls);
    if (!row) return nullptr;
    if (auto* namePtr = row->GetValuePtrByPropertyNameInChain<FText>(STR("OptionName"))) *namePtr = FText(displayName.c_str());
    if (auto* tbPtr = row->GetValuePtrByPropertyNameInChain<UObject*>(STR("OptionNameTextBlock")))
        if (UObject* tb = *tbPtr) umgSetText(tb, displayName.c_str());
    ModPackRow mpr;
    mpr.selector = FWeakObjectPtr(row);
    mpr.modIdx = modIdx;
...
void registerRowStatus(UObject* selector, const wchar_t* staticText, float r, float g, float b, std::function<void(std::wstring&, float&, float&, float&)> dyn = {})
{
    if (!selector) return;
    RowStatus rs;
...
// force-set Visibility via BOTH the UFunction AND a direct
...
// recursive visibility hiding. For composite HBox rows
...
void setRowVisibilityDeep(UObject* w, uint8_t vis)
{
    ...
    auto* singleC = w->GetValuePtrByPropertyNameInChain<UObject*>(STR("Content"));
    if (singleC && *singleC && *singleC != w) setRowVisibilityDeep(*singleC, vis);
}

// move widgets between scrollbox (visible) and a hidden
...
void applyTabContextVisibility()
{
    ...
    if (gpVB && isObjectAlive(gpVB)) setWidgetVisibilityForce(gpVB, 0); // Visible
    if (chVB && isObjectAlive(chVB)) setWidgetVisibilityForce(chVB, 1); // Collapsed
}
...
void onNavTabPressedPost(UObject* context, UFunction* /*fn*/, void* /*parms*/)
{
    ...
    if (ccls == STR("WBP_SettingsScreen_C"))
    ...
    else if (ccls == STR("UI_WBP_NavBar_Build_C"))
    {
        ...
        for (int i = 0; i < 10 && cur; ++i)
        {
            if (safeClassName(cur) == STR("WBP_SettingsScreen_C"))
            {
                settingsScreen = cur;
                break;
            }
...
void forceInjectDualContent(UObject* gameplayTab)
{
    ...
    if (pawn && (pawnCls.find(STR("Dwarf")) != std::wstring::npos || pawnCls.find(STR("FGK")) != std::wstring::npos)) isInGame = true;
    ...
    if (m_dualInjectedFor.Get() != gameplayTab) m_dualContentCached = false;
    if (m_dualInjectedFor.Get() == gameplayTab && m_dualContentCached) return;
    ...
    if (!scrollBoxCls) VLOG(STR("[SettingsUI] CP5 — ScrollBox class NOT found, falling back to VBox\n"));
    if (scrollBoxCls)
    {
        ...
        try
        {
            scrollBox = UObjectGlobals::StaticConstructObject(sbP);
        }
        catch (...)
        {
            scrollBox = nullptr;
        ...
        if (scrollBox)
        {
            ...
            try
            {
                sizeBoxCls = UObjectGlobals::StaticFindObject<UClass*>(nullptr, nullptr, STR("/Script/UMG.SizeBox"));
            }
            catch (...)
            {
            }
            ...
            if (sizeBoxCls)
            {
                ...
                try
                {
                    sizeBox = UObjectGlobals::StaticConstructObject(sbxP);
                }
                catch (...)
                {
                    sizeBox = nullptr;
            ...
            try
            {
                vboxClsForCtx = UObjectGlobals::StaticFindObject<UClass*>(nullptr, nullptr, STR("/Script/UMG.VerticalBox"));
            }
            catch (...)
            {
            }
            if (vboxClsForCtx)
            {
                ...
                try
                {
                    gpVB = UObjectGlobals::StaticConstructObject(gP);
                }
                catch (...)
                {
                }
                ...
                try
                {
                    chVB = UObjectGlobals::StaticConstructObject(cP);
                }
                catch (...)
                {
                }
    ...
    // record that panel setup is done so subsequent
...
skip_panel_setup:;
    ...
    auto addToGameplayCtx = [&](UObject* child) {
        if (gpVB)
            appendToVerticalBox(gpVB, child);
        else if (injectTarget == panel)
            appendToVerticalBox(panel, child);
        else
            addToScrollBox(injectTarget, child);
    };
    // Cheats tab is gone (Option C). Route everything
    ...
    auto addToCheatsCtx = [&](UObject* child) {
        if (gpVB)
            ...
            appendToVerticalBox(chVB, child);
        else if (injectTarget == panel)
            appendToVerticalBox(panel, child);
        else
            addToScrollBox(injectTarget, child);
    };
    auto addToTarget = [&](UObject* child) {
        addToGameplayCtx(child);
    };

    m_dualModGameWidgets.clear();
    m_dualCheatsWidgets.clear();
    // preserve pause-menu entries so SAVE GAME / RENAME /
    ...
    if (isInGame)
    {
        ...
        {
            // spawn ALL gSpecs items (toggles + actions)
            // since everything lives on the Gameplay tab now.
            UObject* row = nullptr;
            // toggle rows use native checkbox.
            if (s.kind == GameOptKind::ToggleNoCollision)
            {
                row = spawnCheckBoxRow(
                        s.label,
                        [this]() {
                            return m_noCollisionWhileFlying;
                        },
                        [this](bool newState) {
            ...
            {
                // action button rows use a custom HBox
                // with a real WBP_FrontEndButton on the right.
                row = spawnActionButtonRow(outer, s.label, s.statusText ? s.statusText : s.label, s.kind);
            }
            if (!row) continue;
            addToTarget(row);
        ...
        }
        {
            ...
            for (int i = 0; i < nT; ++i)
            {
                if (tw[i].kind != TweakKind::SpecialNoCost && tw[i].kind != TweakKind::SpecialInstantCraft) continue;
                const wchar_t* lbl = tw[i].label;
                int idx = i;
                UObject* row = spawnCheckBoxRow(
                        lbl,
                        [this, idx]() -> bool {
                            return idx < (int)m_tweakCurrentIdx.size() && m_tweakCurrentIdx[idx] != 0;
                        },
        ...
        // Combat section (5 buff toggles from F12 Combat).
        if (m_settingsSectionHeadingCls)
        ...
        // Game Settings section (tweak cycles, excluding
            ...
            for (int i = 0; i < nT; ++i)
            {
                ...
                if (m_settingsCarouselCls)
                {
                    ...
                    for (size_t k = 0; k < tw[i].cycleValues.size(); ++k)
                    {
                        ...
                        {
                            wchar_t buf[32];
                            if (tw[i].isMultiplier)
                                swprintf_s(buf, L"%dx", tw[i].cycleValues[k]);
                            else
                                swprintf_s(buf, L"%d", tw[i].cycleValues[k]);
                            lbl = buf;
                        }
        ...
        // God Mode + Buffs sections (relocated to land
        // BEFORE the "Loaded Mods" section per user request).
        {
            int nCheatEntries = 0;
            const CheatEntry* cheats = cheatEntries(nCheatEntries);
            if ((int)m_buffStates.size() != nCheatEntries) m_buffStates.assign(nCheatEntries, false);
            ...
            for (int i = 0; i < nCheatEntries; ++i)
            {
                ...
                UObject* row = spawnBuffToggleRow(i);
                if (!row) continue;
                addToTarget(row); // guaranteed gameplay tab
                m_dualModGameWidgets.push_back(FWeakObjectPtr(row));
            }
            VLOG(STR("[SettingsUI] v0.53 — God Mode + Buffs block ran: nCheatEntries={}\n"), nCheatEntries);
        }

        // Loaded Mods section (per-pack toggles).
        if (m_settingsSectionHeadingCls)
        ...
        {
            if (m_ftGameModEntries.empty()) m_ftGameModEntries = discoverGameMods();
    ...
    for (const auto& s : cSpecs)
    {
        ...
        if (useKeySelectorRows)
        {
            ...
            if (row)
            {
                if (auto* namePtr = row->GetValuePtrByPropertyNameInChain<FText>(STR("OptionName"))) *namePtr = FText(s.label);
                if (auto* tbPtr = row->GetValuePtrByPropertyNameInChain<UObject*>(STR("OptionNameTextBlock")))
        ...
        if (!row)
        {
            ...
            if (!row) continue;
            if (auto* lblPtr = row->GetValuePtrByPropertyNameInChain<FText>(STR("ButtonLabel"))) *lblPtr = FText(s.label);
        ...
        addToTarget(row);
        CheatButton entry;
        entry.widget = FWeakObjectPtr(row);
        entry.kind = s.kind;
        m_cheatsButtons.push_back(entry);
        ...
        if (useKeySelectorRows)
        {
            ...
            {
                registerRowStatus(row, L"OFF", 0.65f, 0.65f, 0.65f, [this](std::wstring& t, float& r, float& g, float& b) {
                    if (m_noCollisionWhileFlying)
                    {
                        t = L"ON";
                        r = 0.31f;
                        g = 0.78f;
            ...
            {
                registerRowStatus(row, L"FIGHT", 0.9f, 0.45f, 0.25f, [this](std::wstring& t, float& r, float& g, float& b) {
    ...
    m_dualInjectedFor = FWeakObjectPtr(gameplayTab);
    m_dualContentCached = true; // early-return on repeat calls
}
...
void applyDualContent()
{
    ...
    for (auto& wp : m_dualNativeChildren)
        setWidgetVisibility(wp.Get(), SHOW);
    for (auto& wp : m_dualModGameWidgets)
        setWidgetVisibility(wp.Get(), SHOW);
}
...
void onModGameOptionClicked(UObject* clickedButton)
{
    ...
    // match by direct widget OR by outer-chain (when the
    ...
    for (auto& g : m_gameOptButtons)
    {
        ...
        case GameOptKind::Rename:
            showRenameDialog_v2(); // uses in-game modal
            break;
        case GameOptKind::RenameModalConfirm:
            confirmRenameDialog(); // Confirm in modal
            break;
        case GameOptKind::RenameModalCancel:
            hideRenameDialog(); // Cancel in modal
            break;
...
// F12-style status text for action rows. Each row using
...
// tweak cycle rows (Gameplay Game Settings section).
// Each row maps to an index in tweakEntries[]. Click →
// cycleTweakValue(idx).
struct TweakCycleRow
{
    FWeakObjectPtr selector;
    int tweakIdx;
};
...
// Loaded Mods rows (Gameplay Loaded Mods section).
...
void injectCheatsTabContent(UObject* legalTab)
{
    ...
    if (m_settingsSectionHeadingCls)
    {
        if (UObject* hd = spawnSectionHeading(STR("Cheats"))) appendToVerticalBox(panel, hd);
    }
...
bool maybeFireCheatFromSelector(UObject* selector)
{
    ...
    for (auto& b : m_cheatsButtons)
    {
        ...
        switch (b.kind)
        {
        case CheatKind::Unlock:
            unlockAllAvailableRecipes();
            break;
        case CheatKind::ReadAll:
            markAllLoreRead();
            break;
        case CheatKind::ClearBuffs:
            clearAllBuffs();
            break;
        case CheatKind::TogglePeace:
            togglePeaceMode();
            break;
        ...
        case CheatKind::RenameChar:
            showRenameDialog_v2();
            break;
        case CheatKind::SaveGame:
            triggerSaveGame();
            break;
        default:
            break;
        }
        if (auto* curPtr = selector->GetValuePtrByPropertyNameInChain<uint8_t>(STR("CurrentSelectedKey"))) std::memset(curPtr, 0, 32);
        return true;
    }
    ...
    // Buff toggle rows (Cheats / Combat).
    ...
    // Tweak cycle rows (Game Settings).
...
{
    if (s_off_uitStride > 0 && s_off_uitName >= 0 && s_off_uitDisplayName >= 0) return true;
    int stride = -1;
    ...
    if (pArrProp)
    {
        if (auto* arrProp = CastField<FArrayProperty>(pArrProp))
        {
            if (auto* innerStructProp = CastField<FStructProperty>(arrProp->GetInner()))
            {
                ...
                {
                    if (auto* ss = static_cast<UScriptStruct*>(sStruct)) stride = static_cast<int>(ss->GetStructureSize());
                    if (auto* p = sStruct->GetPropertyByNameInChain(STR("Name"))) offName = p->GetOffset_Internal();
                    if (auto* p = sStruct->GetPropertyByNameInChain(STR("DisplayName"))) offDisplay = p->GetOffset_Internal();
                    if (auto* p = sStruct->GetPropertyByNameInChain(STR("WidgetClass"))) offWidgetCls = p->GetOffset_Internal();
                    if (auto* p = sStruct->GetPropertyByNameInChain(STR("TabConfig"))) offTabCfg = p->GetOffset_Internal();
                }
            }
        }
    }
    ...
    VLOG(STR("[SettingsUI] FFGKUITab layout resolved: stride={:#x} Name={:#x} "
         ...
         (unsigned)s_off_uitStride,
         (unsigned)s_off_uitName,
         (unsigned)s_off_uitDisplayName,
         (unsigned)s_off_uitWidgetClass,
