...
// F12 panel: 1540px width, wLeft corrected for alignment, Environment tab grouped by bubble with icon click narrowed to 64px
        ...
        UObject* getSlotStateImage(int tb, int slot)
        {
            switch (tb)
            {
            // case 0 (OLD UMG QB bar) removed v6.21.7; case 1 (AB) removed v6.21.15.
        ...
        void refreshKeyLabels()
        {
            ...
            // v6.21.15 - AB key label refresh removed (bar gone)
        }
        ...
        void showRenameDialog_v2()
        {
            ...
            if (!modalCls)
            {
                VLOG(STR("[MoriaCppMod] [Rename v2] in-game modal could not be resolved (see [Rename v2] DIAG above)\n"));
                showErrorBox(L"Rename: in-game modal asset is not loaded in this session. Check the log for [Rename v2] DIAG output.");
                return;
            }
            ...
            if (!modal)
            {
                ...
                showErrorBox(L"Rename: failed to spawn in-game modal widget.");
                return;
            }
        ...
        // v6.21.14 - removed legacy home-rolled showRenameDialog (~314 lines).
        // showRenameDialog_v2 (in-game WBP_UI_RenameWorldModal_C) now
        // surfaces a clear error box if the asset cannot be resolved,
        // rather than silently falling back to a from-scratch UMG popup.
        // v6.21.12 added a UClass scan + diagnostic logging that should
        // make the resolver reliable across game sessions.
