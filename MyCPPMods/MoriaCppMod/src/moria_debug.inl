...
void logGameState(const wchar_t* label)
{
    ...
    auto* s_utilsCDO = UObjectGlobals::StaticFindObject<UObject*>(nullptr, nullptr, STR("/Script/Moria.Default__MoriaUtils"));
    auto* s_fnGetGameState = UObjectGlobals::StaticFindObject<UFunction*>(nullptr, nullptr, STR("/Script/Moria.MoriaUtils:GetMoriaGameState"));
    auto* s_fnGetManager = UObjectGlobals::StaticFindObject<UFunction*>(nullptr, nullptr, STR("/Script/Moria.MoriaUtils:GetManager"));
...

    {
        ...
        if (s_fnGetGameState)
        {
            ...
            for (auto* prop : s_fnGetGameState->ForEachProperty())
            {
                auto name = prop->GetName();
                if (name == STR("WorldContextObject"))
                    s_gsWorldCtx = prop->GetOffset_Internal();
                else if (name == STR("ReturnValue"))
                    s_gsRet = prop->GetOffset_Internal();
            }
        }
        ...
        if (s_fnGetManager)
        {
            ...
            for (auto* prop : s_fnGetManager->ForEachProperty())
            {
                auto name = prop->GetName();
                if (name == STR("WorldContextObject"))
                    s_gmWorldCtx = prop->GetOffset_Internal();
                else if (name == STR("ManagerClass"))
                    s_gmClass = prop->GetOffset_Internal();
                else if (name == STR("ReturnValue"))
                    s_gmRet = prop->GetOffset_Internal();
            }
        }
    ...
    {
        auto* cmClass = UObjectGlobals::StaticFindObject<UClass*>(nullptr, nullptr, STR("/Script/Moria.MorConstructionManager"));
        if (cmClass)
        {
