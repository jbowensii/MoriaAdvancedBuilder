...
static const TweakEntry* tweakEntries(int& outCount)
{
    static const TweakEntry entries[] = {
            ...
            {TweakKind::TweakRow,
             STR("Damage Protection"),
             STR("DamageProtection"),
             /*isFloat=*/true,
             /*isMult=*/true,
             /*gt1=*/false,
             /*filter=*/STR("MorArmorDefinition"),
            ...
            {TweakKind::SpecialNoCost, STR("No Cost"), nullptr, false, false, false, nullptr, {0, 1}}, // OFF / ON

            {TweakKind::SpecialInstantCraft, STR("Instant Craft"), nullptr, false, false, false, nullptr, {0, 1}}, // OFF / ON
    };
    outCount = (int)(sizeof(entries) / sizeof(entries[0]));
    return entries;
}
...
void applyFieldTweak(const wchar_t* fieldName, bool isFloat, bool isMultiplier, bool requiresGt1, const wchar_t* rowStructFilter, int tweakValue, bool useDefault)
{
    ...
    for (UObject* dt : dts)
    {
        ...
        for (const auto& rn : rowNames)
        {
            ...
            {
                double orig = isFloat ? (double)*reinterpret_cast<float*>(fieldAddr) : (double)*reinterpret_cast<int32_t*>(fieldAddr);
            ...
            if (useDefault)
            {
                // Restore original
                if (isFloat)
                    *reinterpret_cast<float*>(fieldAddr) = (float)original;
                else
                    *reinterpret_cast<int32_t*>(fieldAddr) = (int32_t)original;
                rowsTouched++;
                continue;
            }
            ...
            double finalValue = isMultiplier ? original * (double)tweakValue : (double)tweakValue;
            if (isFloat)
                *reinterpret_cast<float*>(fieldAddr) = (float)finalValue;
            else
                *reinterpret_cast<int32_t*>(fieldAddr) = (int32_t)finalValue;
            rowsTouched++;
        }
    ...
    VLOG(STR("[Tweak] field '{}' → {} {} rows across {} tables\n"), fieldName, useDefault ? STR("restored") : STR("set on"), totalRowsTouched, totalTablesTouched);
}
...
void applyNoCostRecipe(bool useDefault)
{
    ...
    for (UObject* dt : dts)
    {
        ...
        for (UStruct* w = rowStruct; w && refundsOff < 0; w = w->GetSuperStruct())
        {
            for (auto* p : w->ForEachProperty())
            {
                ...
                {
                    refundsOff = p->GetOffset_Internal();
                    break;
                }
            }
        }
        ...
        for (const auto& rn : rowNames)
        {
            ...
            if (!arrData || arrNum <= 0 || arrNum > 100)
            { /* still continue to bAllowRefunds */
            }
            else
            {
                for (int32_t i = 0; i < arrNum; ++i)
                {
                    ...
                    if (!m_tweakOriginals.count(key)) m_tweakOriginals[key] = (double)*countAddr;

                    if (useDefault)
                        *countAddr = (int32_t)m_tweakOriginals[key];
                    else
                        *countAddr = 0;
                }
            }
            ...
            if (refundsOff >= 0)
            {
                ...
                if (isReadableMemory(flagAddr, 1))
                {
                    ...
                    if (!m_tweakOriginals.count(key)) m_tweakOriginals[key] = (double)*flagAddr;

                    if (useDefault)
                        *flagAddr = (uint8_t)m_tweakOriginals[key];
                    else
                        *flagAddr = 1;
                }
            }
    ...
    VLOG(STR("[Tweak] NoCost → {} {} rows across {} recipe tables\n"), useDefault ? STR("restored") : STR("zeroed"), totalRows, totalTables);
}
...
void applyInstantCraft(bool useDefault)
{
    ...
    for (UObject* dt : dts)
    {
        ...
        for (const auto& rn : rowNames)
        {
            ...
            if (!m_tweakOriginals.count(key)) m_tweakOriginals[key] = (double)*ctAddr;

            if (useDefault)
                *ctAddr = (float)m_tweakOriginals[key];
            else
                *ctAddr = 0.1f;
            rows++;
        }
    ...
    VLOG(STR("[Tweak] InstantCraft → {} {} rows across {} tables\n"), useDefault ? STR("restored") : STR("set to 0.1s"), totalRows, totalTables);
}
...
void cycleTweakValue(int idx)
{
    ...
    case TweakKind::TweakRow:
        applyFieldTweak(e.fieldName, e.isFloat, e.isMultiplier, e.requiresOriginalGt1, e.rowStructFilter, newVal, useDefault);
        break;
...
void populateGEClassCache()
{
    ...
    UClass* geBaseClass = UObjectGlobals::StaticFindObject<UClass*>(nullptr, nullptr, STR("/Script/GameplayAbilities.GameplayEffect"));
    if (!geBaseClass)
    {
    ...
    UObjectGlobals::ForEachUObject([&](UObject* obj, int32_t /*idx*/, int32_t /*chunk*/) -> LoopAction {
        ...
        bool isClassObj = (cname == L"Class" || cname == L"BlueprintGeneratedClass" || cname == L"DynamicClass");
        ...
        for (UStruct* s = asStruct; s; s = s->GetSuperStruct())
        {
            if (s == geBaseClass)
            {
                ...
                if (name.size() > 2 && name.substr(name.size() - 2) == L"_C") name = name.substr(0, name.size() - 2);
...
bool applyGEByName(const wchar_t* name)
{
    ...
    if (!asc)
    {
        VLOG(STR("[Buff] ASC not found for {}\n"), name);
        return false;
    }
    ...
    if (!fn)
    {
        VLOG(STR("[Buff] BP_ApplyGameplayEffectToSelf UFunction missing\n"));
        return false;
    }
    ...
    *reinterpret_cast<UClass**>(buf.data() + pCls->GetOffset_Internal()) = geCls;
    if (pLevel) *reinterpret_cast<float*>(buf.data() + pLevel->GetOffset_Internal()) = 1.0f;
...
void applySavedCheatsAndTweaks()
{
    ...
    for (int i = 0; i < nTweaks && i < (int)m_tweakCurrentIdx.size(); ++i)
    {
        ...
        {
        case TweakKind::TweakRow:
            applyFieldTweak(e.fieldName, e.isFloat, e.isMultiplier, e.requiresOriginalGt1, e.rowStructFilter, newVal, /*useDefault=*/false);
            break;
        case TweakKind::SpecialNoCost:
            applyNoCostRecipe(false);
            break;
        case TweakKind::SpecialInstantCraft:
            applyInstantCraft(false);
            break;
        default:
            break;
        }
...
void refreshActiveBuffs()
{
    ...
    for (int i = 0; i < count && i < (int)m_buffStates.size(); ++i)
    {
        ...
        if (all[i].effect1)
        {
            applyGEByName(all[i].effect1);
            refreshed++;
        }
        if (all[i].effect2)
        {
            applyGEByName(all[i].effect2);
            refreshed++;
        }
    }
    if (refreshed > 0) VLOG(STR("[Buff] Refreshed {} active effects (periodic tick)\n"), refreshed);
}
...
{
    static const std::vector<std::wstring> prefixes = {L"DEV_",
                                                       L"dev_",
                                                       L"Test_",
                                                       L"TEST_",
                                                       L"test_",
...
void unlock_buildDLCBlockList(std::set<std::wstring>& blockedItems, std::set<std::wstring>& blockedConstructions, std::set<std::wstring>& blockedRunes)
{
    ...
    for (UStruct* w = entTable.rowStruct; w && !handleResolved; w = w->GetSuperStruct())
    {
        for (auto* p : w->ForEachProperty())
        {
            ...
            if (FProperty* inner = arrProp->GetInner())
            {
                ...
                {
                    if (handleStruct->GetPropertiesSize() > 0) handleStride = handleStruct->GetPropertiesSize();
                    int cache = -2;
    ...
    VLOG(STR("[Unlock] DLC filter: {} handles blocked (items={} constructions={} runes={})\n"),
         blockedTotal,
         (int)blockedItems.size(),
         (int)blockedConstructions.size(),
         (int)blockedRunes.size());
}
...
void unlock_collectFromTable(const wchar_t* tableName, const std::set<std::wstring>& blockedByDLC, bool checkResultHandle, std::vector<std::wstring>& outQueue)
{
    DataTableUtil dt;
    ...
    if (checkResultHandle)
    {
        ...
        if (hOff < 0) std::tie(hOff, handleStruct) = unlock_findStructProp(dt.rowStruct, L"ResultConstructionHandle");
...
std::vector<UObject*> markread_findTablesByRowStruct(const wchar_t* rowStructName)
{
    ...
    for (auto* dt : allDTs)
    {
        ...
        try
        {
            if (std::wstring((*rs)->GetName()) == rowStructName) result.push_back(dt);
        }
        catch (...)
        {
        }
...
int markread_markAllByStructType(UObject* screen, const wchar_t* fnName, const wchar_t* paramName, const wchar_t* rowStructName, int structSize)
{
...
void markAllLoreRead()
{
    ...
    // Goals screen — per-entry mark via SetLoreEntryViewed / SetTutorialEntryViewed / SetTipEntryViewed
    {
        ...
        if (!screens.empty() && isObjectAlive(screens[0]))
        {
            ...
            totalMarked += markread_markAllByStructType(gs, STR("SetLoreEntryViewed"), STR("LoreEntry"), STR("MorLoreDefinition"), 0x130);

            // FMorTutorialDefinition = 0x60 bytes
            totalMarked += markread_markAllByStructType(gs, STR("SetTutorialEntryViewed"), STR("TutorialEntry"), STR("MorTutorialDefinition"), 0x60);

            // FMorTipDefinition = 0xA8 bytes
            totalMarked += markread_markAllByStructType(gs, STR("SetTipEntryViewed"), STR("TipEntry"), STR("MorTipDefinition"), 0xA8);
        }
    }
    ...
    // sub-tables may not be covered by the Blueprint MarkAllRead above).
    {
        ...
        if (!screens.empty() && isObjectAlive(screens[0]))
        {
            ...
            totalMarked += markread_markAllByStructType(ls, STR("SetLoreEntryViewed"), STR("LoreEntry"), STR("MorLoreDefinition"), 0x130);
        }
    }
...
void revealEntireMap()
{
        ...
        if (mmMgr)
        {
            ...
            if (zt.bindFromObject(zonesDt, L"ZonesTable"))
            {
                ...
                if (setStartFn)
                {
                    ...
                    if (safeProcessEvent(mmMgr, setStartFn, buf.data())) minimapZonesPushed = static_cast<int>(raw.size());
                }
            }
        }
    }
    ...
    int chaptersDiscovered = 0;
    {
        ...
        for (auto* w : widgets)
        {
            ...
            if (fn && safeProcessEvent(w, fn, nullptr)) chaptersDiscovered++;
        }
    }
