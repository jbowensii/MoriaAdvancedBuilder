...
UObject* getCachedBuildTab()
{
    ...
    if (tab && safeClassName(tab) == L"UI_WBP_Build_Tab_C") return tab;
...
void refreshActionBar()
{
    ...
    if (!actionBar || !isObjectAlive(actionBar)) return;
...
void saveQuickBuildSlots()
{
    ...
    if (!file.is_open())
    {
        VLOG(STR("[MoriaCppMod] saveQuickBuildSlots: failed to open file for writing\n"));
        return;
    }
    ...
    for (int i = 0; i < OVERLAY_BUILD_SLOTS; i++)
    {
        ...
        file << i << "|" << wideToUtf8(m_recipeSlots[i].displayName) << "|" << wideToUtf8(m_recipeSlots[i].textureName) << "|"
...
void loadQuickBuildSlots()
{
    ...
    if (!file.is_open())
    {
        VLOG(STR("[MoriaCppMod] loadQuickBuildSlots: file not found or unreadable\n"));
        return;
    }
    std::string line;
    int loaded = 0;
...
void saveConfig()
{
    ...
    if (!file.is_open())
    {
        VLOG(STR("[MoriaCppMod] saveConfig: failed to open INI for writing\n"));
        return;
    }
    ...
    for (int i = 0; i < BIND_COUNT; i++)
    {
        ...
        if (!s_bindings[i].enabled) file << iniKey << "_Enabled = false\n";
    }
    ...
    for (int i = 0; i < 8; ++i)
    {
        ...
        file << "QuickBuild" << (i + 1) << "Set = " << prefix << wideToUtf8(keyName(s_setBindings[i].vk)) << "\n";
    }
    ...
    if (hasCustomPos)
    {
        ...
        if (m_rotDispPosX >= 0.0f) file << "RotDisplayX = " << std::fixed << std::setprecision(4) << m_rotDispPosX << "\n";
        if (m_rotDispPosY >= 0.0f) file << "RotDisplayY = " << std::fixed << std::setprecision(4) << m_rotDispPosY << "\n";
    }
        ...
        while (std::getline(file, line))
        {
            ...
            else if (auto* kv = std::get_if<ParsedIniKeyValue>(&parsed))
            {
                if (strEqualCI(section, "KeybindingsSet"))
                {
                    ...
                    std::string k = kv->key;
                    if (k.size() > 11 && strEqualCI(k.substr(0, 10), "QuickBuild") && strEqualCI(k.substr(k.size() - 3), "Set"))
                    {
                        int slot = -1;
                        try
                        {
                            slot = std::stoi(k.substr(10, k.size() - 13)) - 1;
                        }
                        catch (...)
                        {
                        }
                        if (slot >= 0 && slot < 8)
                        {
                            ...
                            for (;;)
                            {
                                if (val.size() >= 6 && strEqualCI(val.substr(0, 6), "SHIFT+"))
                                {
                                    mods |= 0x01;
                                    val = val.substr(6);
                                }
                                else if (val.size() >= 5 && strEqualCI(val.substr(0, 5), "CTRL+"))
                                {
                                    mods |= 0x02;
                                    val = val.substr(5);
                                }
                                else if (val.size() >= 4 && strEqualCI(val.substr(0, 4), "ALT+"))
                                {
                                    mods |= 0x04;
                                    val = val.substr(4);
                                }
                            ...
                            if (vk)
                            {
                                s_setBindings[slot].vk = *vk;
                                s_setBindings[slot].modBits = mods;
                            }
                        }
                    }
                }
                    ...
                    for (int i = 0; i < nTweaks; ++i)
                    {
                        ...
                        if (strEqualCI(entryKey, kv->key))
                        {
                            try
                            {
                                ...
                                if (idx >= 0 && idx < (int)tweaks[i].cycleValues.size()) m_tweakCurrentIdx[i] = idx;
                            }
                            catch (...)
                            {
                            }
...
bool extractAndSaveIcon(UObject* widget, const std::wstring& textureName, const std::wstring& outPath, UObject* textureOverride = nullptr)
{
    ...
    try
    {
        ...
        auto* createRTFn = UObjectGlobals::StaticFindObject<UFunction*>(nullptr, nullptr, STR("/Script/Engine.KismetRenderingLibrary:CreateRenderTarget2D"));
        auto* beginDrawFn =
        ...
        auto* endDrawFn = UObjectGlobals::StaticFindObject<UFunction*>(nullptr, nullptr, STR("/Script/Engine.KismetRenderingLibrary:EndDrawCanvasToRenderTarget"));
        ...
        UObject* worldCtx = nullptr;
        {
            ...
            findAllOfSafe(STR("PlayerController"), pcs);
            for (auto* pc : pcs)
            {
                if (pc)
                {
        ...
        UObject* renderTarget = nullptr;
        {
            ...
            if (!pWC || !pRV)
            {
                VLOG(STR("[MoriaCppMod] [Icon] CreateRenderTarget2D: critical params missing\n"));
                return false;
            }
        ...
        std::vector<uint8_t> beginParams;
        {
            ...
            if (!bWC || !bRT)
            {
                VLOG(STR("[MoriaCppMod] [Icon] BeginDraw: critical params missing\n"));
                return false;
            }
...

        {
            ...
            if (!dTex)
            {
                VLOG(STR("[MoriaCppMod] [Icon] K2_DrawTexture: RenderTexture param missing, skipping draw\n"));
            }
            else
            {
...

        {
            ...
            if (!eWC || !eRT)
            {
                VLOG(STR("[MoriaCppMod] [Icon] ExportRT: critical params missing\n"));
            }
            else
            {
...
std::wstring extractIconTextureName(UObject* widget)
{
    if (!widget)
    {
        QBLOG(STR("[MoriaCppMod] [QB] extractIconTextureName: widget=null\n"));
        return L"";
    }
    try
    {
        ...
        if (s_off_icon < 0)
        {
            QBLOG(STR("[MoriaCppMod] [QB] extractIconTextureName: s_off_icon not resolved\n"));
            return L"";
        }
        ...
        if (s_off_brush < 0)
        {
            QBLOG(STR("[MoriaCppMod] [QB] extractIconTextureName: s_off_brush not resolved\n"));
            return L"";
        }
        uint8_t* imgBase = reinterpret_cast<uint8_t*>(iconImg);
        UObject* brushResource = *reinterpret_cast<UObject**>(imgBase + s_off_brush + brushResourceObj());
        QBLOG(STR("[MoriaCppMod] [QB] extractIconTextureName: brushResource={:p} readable={}\n"),
              (void*)brushResource,
              brushResource ? isReadableMemory(brushResource, 64) : false);
        ...
        if (s_off_texParamValues < 0)
        {
            QBLOG(STR("[MoriaCppMod] [QB] extractIconTextureName: s_off_texParamValues not resolved\n"));
            return L"";
        }
        uint8_t* midBase = reinterpret_cast<uint8_t*>(brushResource);
        uint8_t* arrData = *reinterpret_cast<uint8_t**>(midBase + s_off_texParamValues);
...
void assignRecipeSlot(int slot)
{
    ...
    QBLOG(STR("[MoriaCppMod] [QB] assignRecipeSlot: buildTab={} hasBuildObject={}\n"),
          buildTab ? STR("YES") : STR("NO"),
          hasBuildObject ? STR("true") : STR("false"));
...
void quickBuildSlot(int slot)
{
    ...
    if (!m_recipeSlots[slot].used)
    {
        std::wstring msg = L"F" + std::to_wstring(slot + 1) + L" empty \x2014 select recipe in B menu, then " + modifierName(s_modifierVK) + L"+F" +
                           std::to_wstring(slot + 1);
        showErrorBox(msg);
        return;
    }

    if (m_handleResolvePhase == HandleResolvePhase::Priming || m_handleResolvePhase == HandleResolvePhase::Resolving)
    {
