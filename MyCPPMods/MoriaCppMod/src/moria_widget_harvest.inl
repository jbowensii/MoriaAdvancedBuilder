...
inline std::string safeGetWidgetMemberName(UObject* widget)
{
    ...
    try
    {
        return jsonEscape(widget->GetName());
    }
    catch (...)
    {
        return "";
...
inline std::string harvestSlot(UObject* widget)
{
    ...
    auto readF4 = [&](const wchar_t* name, std::string label) {
        ...
        {
            char buf[160];
            snprintf(buf, sizeof(buf), ",\"%s\":[%g,%g,%g,%g]", label.c_str(), p[0], p[1], p[2], p[3]);
            out += buf;
        }
    };
...
inline std::string harvestWidgetExtras(UObject* widget)
{
    ...
    if (getTextFn)
    {
        ...
        try
        {
            safeProcessEvent(widget, getTextFn, p.data());
        }
        catch (...)
        {
        }
    ...
    if (brushPtrA && *brushPtrA) out += ",\"brush\":\"" + safeGetPathName(*brushPtrA) + "\"";
    ...
    {
        char buf[64];
        snprintf(buf, sizeof(buf), ",\"WidthOverride\":%g", *w);
        out += buf;
    }
    ...
    {
        char buf[64];
        snprintf(buf, sizeof(buf), ",\"HeightOverride\":%g", *h);
        out += buf;
    }
    ...
    {
        char buf[64];
        snprintf(buf, sizeof(buf), ",\"Visibility\":%u", static_cast<unsigned>(*vis));
        out += buf;
    }
    ...
    if (fontPtr && *fontPtr) out += ",\"font\":\"" + safeGetPathName(*fontPtr) + "\"";
    auto* fontSize = widget->GetValuePtrByPropertyNameInChain<int32_t>(STR("Font.Size"));
    if (fontSize)
    {
        char buf[64];
        snprintf(buf, sizeof(buf), ",\"FontSize\":%d", *fontSize);
        out += buf;
    }
...
inline void emitWidgetJson(UObject* widget, int depth, std::ofstream& out)
{
    ...
    if (slotsAddr)
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            if (contentPtr && *contentPtr && isObjectAlive(*contentPtr)) children.push_back(*contentPtr);
        }
    }
    ...
    if (children.empty())
    {
        ...
        if (singleContent && *singleContent && isObjectAlive(*singleContent)) children.push_back(*singleContent);
    }
...
inline void harvestOneWidget(const wchar_t* widgetClassShortName)
{
    ...
    if (!pc)
    {
        VLOG(STR("[WidgetHarvest] no PC, abort\n"));
        return;
    }
    ...
    if (!createFn || !wblClass)
    {
        VLOG(STR("[WidgetHarvest] WBL missing\n"));
        return;
    }
...
inline void harvestJoinWorldWidgets()
{
    ...
    showOnScreen(L"Harvesting Join World widgets... (see Mods/MoriaCppMod/widget-harvest/)", 3.0f, 0.4f, 0.9f, 0.4f);
