...
void jw_applyCapturedFont(UObject* textBlock, const uint8_t* fontBuf, int32_t sizeOverride = -1)
{
    ...
    if (sizeOverride > 0) std::memcpy(local + fontSizeOff(), &sizeOverride, sizeof(int32_t));
...
bool jw_applyFontByAssetPath(UObject* textBlock, const wchar_t* assetPath, int32_t fontSize, const wchar_t* typefaceName = STR(""))
{
...
bool jw_setImageBrush(UObject* img, UObject* texture, UFunction* setBrushFn, float w, float h, const wchar_t* tag = STR("?"))
{
    ...
    try
    {
        umgSetBrushNoMatch(img, texture, setBrushFn);
    }
    catch (...)
    {
    }
...
UObject* jw_loadAssetBlocking(const wchar_t* path)
{
    ...
    try
    {
        auto* fn = UObjectGlobals::StaticFindObject<UFunction*>(nullptr, nullptr, STR("/Script/Engine.KismetSystemLibrary:LoadAsset_Blocking"));
        if (!fn)
        {
            VLOG(STR("[JoinWorldUI] jw_loadAssetBlocking('{}'): UFunction not found\n"), path);
            return nullptr;
        }
        auto* cls = UObjectGlobals::StaticFindObject<UClass*>(nullptr, nullptr, STR("/Script/Engine.KismetSystemLibrary"));
        if (!cls)
        {
        ...
        VLOG(STR("[JoinWorldUI] jw_loadAssetBlocking('{}'): FName ci={} num={}, parmSize={} assetOff={} retOff={}\n"),
             ...
             num,
             sz,
             pAsset->GetOffset_Internal(),
             pRet->GetOffset_Internal());
        if (ci == 0)
        {
...
void cacheJoinWorldClassRefs(UObject* userWidget)
{
    ...
    VLOG(STR("[JoinWorldUI] class cache: feBtn={:p} bigBtn={:p} gameData={:p} sessHist={:p} advJoin={:p} lowerThird={:p} netAlert={:p} ctrlPrompt={:p} "
             "txtHdr={:p}\n"),
         (void*)m_jwCls_FrontEndButton,
         (void*)m_jwCls_CraftBigButton,
         (void*)m_jwCls_GameDataPanel,
         (void*)m_jwCls_SessionHistoryList,
         (void*)m_jwCls_AdvancedJoinPanel,
         (void*)m_jwCls_LowerThird,
         (void*)m_jwCls_NetworkAlert,
         (void*)m_jwCls_ControlPrompt,
         (void*)m_jwCls_TextHeader);
    VLOG(STR("[JoinWorldUI] font capture: title={} breadcrumb={} subtitle={} histHeader={}\n"),
         m_jwFontTitleCaptured ? 1 : 0,
         m_jwFontBreadcrumbCaptured ? 1 : 0,
         m_jwFontSubtitleCaptured ? 1 : 0,
         m_jwFontHistoryHeaderCaptured ? 1 : 0);
    ...
    if (m_jwIconTexSearch && isObjectAlive(m_jwIconTexSearch))
    {
        ...
        try
        {
            iconFull = m_jwIconTexSearch->GetFullName();
        }
        catch (...)
        {
        }
        try
        {
            iconPath = m_jwIconTexSearch->GetPathName();
        }
        catch (...)
        {
        }
    ...
    try
    {
        ...
        for (auto* t : textures)
        {
            ...
            if (nm.find(L"T_UI_Btn_P1_Up") != std::wstring::npos || nm.find(L"T_UI_Btn_P2_Up") != std::wstring::npos ||
    ...
    auto resolve = [&](const wchar_t* path) -> UObject* {
        ...
        try
        {
            o = UObjectGlobals::StaticFindObject<UObject*>(nullptr, nullptr, path);
        }
        catch (...)
        {
        }
    ...
    if (!m_jwTexBtnP1Up) m_jwTexBtnP1Up = resolve(STR("/Game/UI/Textures/_Shared/T_UI_Btn_P1_Up.T_UI_Btn_P1_Up"));
    if (!m_jwTexBtnP2Up) m_jwTexBtnP2Up = resolve(STR("/Game/UI/Textures/_Shared/T_UI_Btn_P2_Up.T_UI_Btn_P2_Up"));
    if (!m_jwTexBtnCTADisabled) m_jwTexBtnCTADisabled = resolve(STR("/Game/UI/Textures/_Shared/T_UI_Btn_CTA_Disabled.T_UI_Btn_CTA_Disabled"));
...
UObject* jw_spawnFEButton(const wchar_t* label, UObject* iconTex, float W, float H, UClass* sizeBoxClass, UObject* outer)
{
    ...
    for (const wchar_t* n : {STR("HoverImage"), STR("FocusImage"), STR("DisabledImage")})
    {
...
void jw_setFrontEndButtonLabel(UObject* btn, const std::wstring& label)
{
    ...
    if (btnTxtPtr && *btnTxtPtr) umgSetText(*btnTxtPtr, label);
}
