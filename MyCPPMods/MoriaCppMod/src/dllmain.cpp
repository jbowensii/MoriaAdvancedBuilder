...
namespace MoriaMods
{
    ...
    static std::wstring keyName(uint8_t vk)
    {
        ...
        switch (vk) {
            ...
            default: {
                wchar_t buf[16];
                if (vk >= 0x30 && vk <= 0x39) { swprintf_s(buf, L"%c", (wchar_t)vk); return buf; }
                if (vk >= 0x41 && vk <= 0x5A) { swprintf_s(buf, L"%c", (wchar_t)vk); return buf; }
    ...
    static HWND findGameWindow()
    {
        ...
        struct FindData { HWND best; int bestArea; };
    ...
    static void renderOverlay(HWND hwnd)
    {
        ...
        if (s_overlay.csInit) {
            ...
            for (int i = 0; i < OVERLAY_SLOTS; i++) localSlots[i] = s_overlay.slots[i];
            ...
            for (int i = 0; i < OVERLAY_SLOTS; i++) {
                ...
                if (i < OVERLAY_BUILD_SLOTS) {
                    if (localSlots[i].used && localSlots[i].icon) {
                        ...
                        gfx.DrawImage(localSlots[i].icon,
                            Gdiplus::Rect(sx + iconPad, sy + iconPad,
                ...
                if (i == 11) {
                    ...
                    for (int t = 0; t < nTeeth; t++) {
                        ...
                        gearPts.push_back({gcx + tipR * cosf(ctrAngle - halfTip),
                                           gcy + tipR * sinf(ctrAngle - halfTip)});
                        gearPts.push_back({gcx + tipR * cosf(ctrAngle + halfTip),
                                           gcy + tipR * sinf(ctrAngle + halfTip)});
                        // Flat root valley (inner)
                        gearPts.push_back({gcx + rootR * cosf(gapCtr - halfRoot),
                                           gcy + rootR * sinf(gapCtr - halfRoot)});
                        gearPts.push_back({gcx + rootR * cosf(gapCtr + halfRoot),
                                           gcy + rootR * sinf(gapCtr + halfRoot)});
                    }
                ...
                Gdiplus::RectF labelRect((float)sx, (float)(sy + slotSize + 1),
                                         (float)slotSize, (float)labelH);
    ...
    static void copyTargetInfoToClipboard(HWND hwnd)
    {
        ...
        copyText = L"Class: " + s_targetInfo.actorClass + L"\r\n"
                 ...
                 + L"Display: " + s_targetInfo.displayName + L"\r\n"
    ...
    static void renderTargetInfo(HWND hwnd)
    {
        ...
        struct { const wchar_t* label; const std::wstring& value; } singleRows[] = {
            { L"Class:  ", dClass },
            { L"Name:   ", dName },
            { L"Display:", dDisplay },
        };
        ...
        Gdiplus::SolidBrush buildBrush(dBuildable
            ? Gdiplus::Color(230, 80, 220, 80)   // green for yes
        ...
        SetWindowPos(hwnd, HWND_TOPMOST, panelX, panelY, panelW, panelH,
    ...
    static LRESULT CALLBACK targetInfoWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
    {
        ...
        case WM_LBUTTONDOWN: {
            ...
            if (mx >= s_targetInfo.closeBtnRect.X &&
                mx <= s_targetInfo.closeBtnRect.X + s_targetInfo.closeBtnRect.Width &&
                my >= s_targetInfo.closeBtnRect.Y &&
                my <= s_targetInfo.closeBtnRect.Y + s_targetInfo.closeBtnRect.Height) {
                s_targetInfo.visible = false;
                return 0;
            }
            // Hit-test copy button
            if (mx >= s_targetInfo.copyBtnRect.X &&
                mx <= s_targetInfo.copyBtnRect.X + s_targetInfo.copyBtnRect.Width &&
                my >= s_targetInfo.copyBtnRect.Y &&
    ...
    static void renderConfig(HWND hwnd)
    {
...

        {
            ...
            for (int t = 0; t < CONFIG_TAB_COUNT; t++) {
                ...
                gfx.DrawString(CONFIG_TAB_NAMES[t], -1, &tabFont, tabTextRect, &centerFmt,
                    t == s_config.activeTab ? &tabTextBrush : &tabTextDimBrush);
            }
            ...
            if (s_config.activeTab == 2) {
                ...
                for (int b = 0; b < BIND_COUNT; b++) {
                    ...
                    if (!lastSection || wcscmp(lastSection, s_bindings[b].section) != 0) {
                        ...
                        cy += rowH;
                        gfx.DrawLine(&sepPen, pad, cy - static_cast<int>(4 * scale),
                                     configW - pad - scrollbarW, cy - static_cast<int>(4 * scale));
                    }

                    // Label
                    Gdiplus::RectF labelRect((float)(pad + static_cast<int>(12 * scale)), (float)cy,
                                             (float)(configW - pad * 2 - keyBoxW - scrollbarW - static_cast<int>(24 * scale)), (float)rowH);
                ...
                Gdiplus::RectF modLabelRect((float)(pad + static_cast<int>(12 * scale)), (float)cy,
                                            (float)(configW - pad * 2 - keyBoxW - scrollbarW - static_cast<int>(24 * scale)), (float)rowH);
                ...
                for (int ti = 0; ti < 3; ti++) {
                    Gdiplus::RectF lblRect((float)(pad + static_cast<int>(12 * scale)), (float)cy,
                                           (float)(configW - pad * 2 - toggleW - scrollbarW - static_cast<int>(24 * scale)), (float)rowH);
                    ...
                    Gdiplus::RectF statusRect((float)(toggleX - static_cast<int>(36 * scale)), (float)cy,
                                              (float)(static_cast<int>(32 * scale)), (float)rowH);
                    ...
                    gfx.DrawString(toggles[ti].state ? L"ON" : L"OFF", -1, &rowFont, statusRect, &rightFmt,
                                   toggles[ti].state ? &statusOnBrush : &statusOffBrush);

                    cy += rowH;

                    Gdiplus::RectF descRect((float)(pad + static_cast<int>(24 * scale)), (float)cy,
                                            (float)(configW - pad * 2 - scrollbarW - static_cast<int>(24 * scale)), (float)rowH);
                ...
                Gdiplus::RectF contentRect((float)pad, (float)(cy),
                                           (float)(configW - pad * 2 - scrollbarW), (float)(configH - cy - pad));
            ...
            if (s_config.activeTab == 1 && my > contentY) {
                ...
                for (int ti = 0; ti < 3; ti++) {
                    ...
                    if (mx >= toggleX && mx <= toggleX + toggleW && logicalMy >= toggleY && logicalMy <= toggleY + toggleH) {
                        if (ti == 0) s_config.pendingToggleFreeBuild = true;
                        else if (ti == 1) s_config.pendingToggleFreeCraft = true;
                        else if (ti == 2) s_config.pendingToggleInstantCraft = true;
                        renderConfig(hwnd);
                        return 0;
                    }
                    cy += rowH;
    ...
    private:
        ...
        void dumpAimedActor()
        {
            ...
            if (getDispFn) {
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] GetDisplayName found, parmsSize={}\n"), getDispFn->GetParmsSize());
            ...
            if (displayName.empty()) {
                ...
                if (cleaned.size() > 3 && cleaned.substr(0, 3) == L"BP_")
                    cleaned = cleaned.substr(3);
                if (cleaned.size() > 2 && cleaned.substr(cleaned.size() - 2) == L"_C")
                    cleaned = cleaned.substr(0, cleaned.size() - 2);
                for (auto& c : cleaned) { if (c == L'_') c = L' '; }
                displayName = cleaned;
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] Display name fallback: '{}'\n"), displayName);
            }

            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] Actor: {} | Class: {}\n"), actorName, actorClassName);
            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] Display: {}\n"), displayName);
            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] Path: {}\n"), assetPath);
            ...
            if (actorCls) {
                ...
                for (auto* prop : actorStruct->ForEachPropertyInChain()) {
                    ...
                    if (propName.find(L"MorConstructionSnap") != std::wstring::npos ||
                        propName.find(L"MorConstructionStability") != std::wstring::npos ||
                ...
                if (!isBuildable) {
                    for (auto* super = actorCls->GetSuperStruct(); super; super = super->GetSuperStruct()) {
                        ...
                        if (superName.find(L"BaseArchitectureBreakable") != std::wstring::npos ||
                            superName.find(L"CraftingStation") != std::wstring::npos ||
                ...
                if (isBuildable) {
                    ...
                    if (recipeRef.size() > 2 && recipeRef.substr(recipeRef.size() - 2) == L"_C")
                        recipeRef = recipeRef.substr(0, recipeRef.size() - 2);
                }
            }

            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] Buildable: {} Recipe: {}\n"),
            ...
            std::wstring dtRowName;
            {
                ...
                if (dumpFile.is_open()) {
                    ...
                    if (!dtConst) {
                        ...
                        struct { uint8_t* Data; int32_t Num; int32_t Max; } elemArray{};
                        ...
                        dumpFile << L"\n=== RESULTS: " << elemArray.Num << L" rows scanned, "
                                 << matchCount << L" matches ===\n";
                    ...
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] DT_Constructions scan -> actor_dump.txt\n"));
                }
            }
            ...
            if (!dtDisplayName.empty()) {
                ...
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] DT display name: '{}' (row '{}')\n"), displayName, dtRowName);
            }
            ...
            if (isBuildable && !displayName.empty()) {
                ...
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [TargetBuild] Stored target: name='{}' recipeRef='{}' row='{}'\n"),
                    m_targetBuildName, m_targetBuildRecipeRef, m_targetBuildRowName);
            } else {
        ...
        bool discoverBagHandle(UObject* invComp)
        {
            ...
            struct ParamInfo { int offset{-1}; int size{0}; };
            auto findParam = [](UFunction* func, const wchar_t* name) -> ParamInfo {
                for (auto* prop : func->ForEachProperty()) {
                    if (std::wstring(prop->GetName()) == name)
                        return {prop->GetOffset_Internal(), prop->GetSize()};
                }
                return {};
            };
            ...
            if (itemsList.Data && itemsList.Num > 0 && itemsList.Num < 500 &&
                isReadableMemory(itemsList.Data, itemsList.Num * ITEMINSTANCE_SIZE))
            {
                for (int i = 0; i < itemsList.Num; i++) {
                    ...
                    if (cls && isReadableMemory(cls, 64)) {
                        try {
                            ...
                            idToClass[id] = name;
                            if (name.find(STR("BodyInventory")) != std::wstring::npos)
                                bodyInvClass = cls;
                        }
                        catch (...) {}
                    }
            ...
            if (!m_ihfCDO) {
                m_ihfCDO = UObjectGlobals::StaticFindObject<UObject*>(
                    nullptr, nullptr, STR("/Script/FGK.Default__ItemHandleFunctions"));
            }
            ...
            if (contData && contNum > 0 && contNum < 32 &&
                isReadableMemory(contData, contNum * handleSize))
            {
                for (int i = 0; i < contNum; i++) {
                    ...
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] Container[{}] ID={} slots={} class='{}'\n"), i, cId, maxSlots, cName);
                }
            }
            ...
            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] Found {} BodyInventory containers\n"),
                    ...
                    if (aiItem.offset >= 0 && aiCount.offset >= 0 && aiRet.offset >= 0) {
                        for (int i = 0; i < 2; i++) {
                            ...
                            *reinterpret_cast<int32_t*>(aiBuf.data() + aiCount.offset) = 1;
                            if (aiMethod.offset >= 0)
                                aiBuf[aiMethod.offset] = 1;  // EAddItem::Create
                            ...
                            Output::send<LogLevel::Warning>(
                                STR("[MoriaCppMod] Created stash BodyInventory #{} — ID={}\n"),
                                i + 1, newId);
                        }
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] Now have {} BodyInventory containers (1 hotbar + 2 stash)\n"),
                            m_bodyInvHandles.size());
                    } else {
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] AddItem param offsets not found (Item={} Count={} Ret={})\n"),
                            aiItem.offset, aiCount.offset, aiRet.offset);
                    }
                } else {
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] AddItem function not found on inventory component\n"));
                }
            } else if (m_bodyInvHandles.size() == 1) {
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] Only 1 BodyInventory but class not found — cannot create stash\n"));
            }
        ...
        void probeInventoryHotbar()
        {
            ...
            UObject* playerChar = nullptr;
            {
                ...
                for (auto* a : actors) {
                    ...
                    if (cls.find(STR("Dwarf")) != std::wstring::npos ||
                        cls.find(STR("FGK")) != std::wstring::npos) {
                        playerChar = a;
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [Inv] Found player char: {} '{}'\n"),
            ...
            Output::send<LogLevel::Warning>(STR("[MoriaCppMod] [Inv] --- Player Character Components ---\n"));
            {
                ...
                if (!getCompsFunc)
                    getCompsFunc = playerChar->GetFunctionByNameInChain(STR("GetComponents"));
                ...
                for (auto* c : allComps) {
                    ...
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] [Inv]   comp[{}]: {} '{}'\n"),
                        compIdx++, compCls, compName);

                    // Check for inventory-related components
                    std::string narrow;
                    for (wchar_t ch : compCls) narrow.push_back(static_cast<char>(std::tolower(ch)));
                    bool isInventory = (narrow.find("inventor") != std::string::npos ||
                                       narrow.find("storage") != std::string::npos ||
                                       narrow.find("hotbar") != std::string::npos ||
                                       narrow.find("equip") != std::string::npos ||
                                       narrow.find("item") != std::string::npos ||
                                       narrow.find("container") != std::string::npos);

                    if (isInventory) {
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [Inv]   *** INVENTORY COMPONENT FOUND: {} ***\n"), compCls);
                                ...
                                if (sz == 8) {
                                    ...
                                    if (isReadableMemory(base + off, 8)) {
                                        ...
                                        if (ptr && isReadableMemory(ptr, 64)) {
                                            try {
                                                extra = L" -> " + safeClassName(ptr) + L" '" +
                                                        std::wstring(ptr->GetName()) + L"'";
                                            } catch (...) { extra = L" -> (err)"; }
                                        } else if (ptr) {
                ...
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [Inv] Total components on player: {}\n"), compIdx);
            }
            ...
            // --- Step 2b: Read hotbar items via GetItemForHotbarSlot ---
            {
                ...
                if (invComp) {
                    ...
                    int hotbarSize = 8;
                    if (hotbarSizeFunc) {
                        struct { int32_t Ret{0}; } hs{};
                        invComp->ProcessEvent(hotbarSizeFunc, &hs);
                        hotbarSize = hs.Ret;
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [Inv] HotbarSize = {}\n"), hotbarSize);
                    }
                    ...
                    int epicIdx = -1;
                    if (epicIdxFunc) {
                        struct { int32_t Ret{0}; } ei{};
                        invComp->ProcessEvent(epicIdxFunc, &ei);
                        epicIdx = ei.Ret;
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [Inv] HotbarEpicItemIndex = {}\n"), epicIdx);
                    }
                    ...
                    if (getItemFunc) {
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [Inv] GetItemForHotbarSlot ({}B) params:\n"),
                            getItemFunc->GetParmsSize());
                        for (auto* prop : getItemFunc->ForEachProperty()) {
                            if (!prop) continue;
                            Output::send<LogLevel::Warning>(
                                STR("[MoriaCppMod] [Inv]   param: {} @{} size={}\n"),
                                std::wstring(prop->GetName()), prop->GetOffset_Internal(), prop->GetSize());
                        }
                        ...
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [Inv] Items.List TArray @0x330: data={:p} num={} max={}\n"),
                            static_cast<void*>(itemsList.Data), itemsList.Num, itemsList.Max);
                        ...
                        if (itemsList.Data && itemsList.Num > 0 && itemsList.Num < 500) {
                            ...
                            if (isReadableMemory(itemsList.Data, totalBytes)) {
                                Output::send<LogLevel::Warning>(
                                    STR("[MoriaCppMod] [Inv] Reading {} item instances...\n"), itemsList.Num);
                                for (int i = 0; i < itemsList.Num; i++) {
                                    uint8_t* entry = itemsList.Data + i * ITEMINSTANCE_SIZE;
                                    ...
                                    if (itemClass && isReadableMemory(itemClass, 64)) {
                                        try { clsName = std::wstring(itemClass->GetName()); }
                                        catch (...) { clsName = L"(err)"; }
                                    }

                                    idMap[id] = {clsName, count, slot, id, durability};
                                    Output::send<LogLevel::Warning>(
                                        STR("[MoriaCppMod] [Inv]   item[{}]: id={} slot={} count={} dur={:.1f} class='{}'\n"),
                                        i, id, slot, count, durability, clsName);
                                }
                            }
                        }
                        ...
                        for (int slot = 0; slot <= hotbarSize; slot++) {
                            ...
                            *reinterpret_cast<int32_t*>(buf) = querySlot;
                            ...
                            std::wstring label = isEpic
                                ...
                                : std::format(L"Slot[{}]", querySlot);

                            if (itemId == 0 && payload == 0) {
                                Output::send<LogLevel::Warning>(
                                    STR("[MoriaCppMod] [Inv] {}: (empty)\n"), label);
                            } else {
                                auto it = idMap.find(itemId);
                                if (it != idMap.end()) {
                                    const auto& info = it->second;
                                    Output::send<LogLevel::Warning>(
                                        STR("[MoriaCppMod] [Inv] {}: '{}' x{} (dur={:.0f}, id={})\n"),
                                        label, info.className, info.count, info.durability, itemId);
                                } else {
                                    Output::send<LogLevel::Warning>(
                                        STR("[MoriaCppMod] [Inv] {}: id={} payload={} (NOT in ClientItems)\n"),
                        ...
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [Inv] GetItemForHotbarSlot NOT FOUND\n"));
                    }
                } else {
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] [Inv] Could not find MorInventoryComponent\n"));
                }
            }
        ...
        void clearHotbarTick()
        {
            ...
            if (m_clearHotbarWait > 0) { m_clearHotbarWait--; return; }
            if (m_clearHotbarCount + m_clearHotbarDropped >= 8) {
                std::wstring msg = std::format(L"Hotbar cleared: {} to bag, {} dropped",
                    m_clearHotbarCount, m_clearHotbarDropped);
            ...
            if (!playerChar) { m_clearingHotbar = false; return; }

            auto* invComp = findPlayerInventoryComponent(playerChar);
            if (!invComp) { m_clearingHotbar = false; return; }
            ...
            if (!getSlotFunc || !moveFunc || !dropFunc) { m_clearingHotbar = false; return; }
                ...
                *reinterpret_cast<int32_t*>(slotBuf.data() + slotInput.offset) = slot;
                ...
                if (canMove) {
                    ...
                    *reinterpret_cast<int32_t*>(dropBuf.data() + dc.offset) = 999999;
                    ...
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] Slot {} (id={}) → dropped (bag full)\n"), slot, itemId);
                }
        ...
        void patchBodyInventoryStorage()
        {
            ...
            for (auto* dt : dataTables) {
                ...
                try {
                    ...
                    if (name.find(STR("DT_Storage")) != std::wstring::npos) {
                        dtStorage = dt;
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] Found DataTable: '{}'\n"), name);
                        break;
                    }
                } catch (...) { continue; }
            }
            ...
            if (!elemArray.Data || elemArray.Num <= 0 || elemArray.Num > 1000) {
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] RowMap TArray invalid: Num={}, Max={}\n"),
                    elemArray.Num, elemArray.Max);
                return;
            }

            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] RowMap has {} entries\n"), elemArray.Num);
            ...
            for (int i = 0; i < elemArray.Num; i++) {
                ...
                if (entryName == targetName) {
                    ...
                    if (rowData && isReadableMemory(rowData, 0x58)) {
                        foundRow = rowData;
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] Found Dwarf.BodyInventory row at entry {}\n"), i);
                    }
                    break;
                }
            }
            ...
            if (!foundRow) {
                ...
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] Dwarf.BodyInventory not found. Logging row names:\n"));
                for (int i = 0; i < std::min(elemArray.Num, 30); i++) {
                    ...
                    try {
                        ...
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod]   Row[{}]: (FName unreadable)\n"), i);
                    }
                }
                return;
            }
            ...
            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] Current: InventoryWidth={}, InventoryHeight={}\n"),
                oldWidth, oldHeight);
            ...
            if (oldHeight < newHeight) {
                ...
                *heightPtr = newHeight;
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] Patched BodyInventory: {}x{} → {}x{}\n"),
                    oldWidth, oldHeight, newWidth, newHeight);
            } else {
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] BodyInventory already {}x{} (h >= {}), no patch\n"),
        ...
        void swapToolbar()
        {
          ...
          try {
            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] [Swap] State: active={} clearing={} bodyInvHandle.empty={} handles.size={} charLoaded={}\n"),
                m_swap.active, m_clearingHotbar, m_bodyInvHandle.empty(),
                m_bodyInvHandles.size(), m_characterLoaded);
            ...
            if (m_bodyInvHandles.size() < 3) {
                showOnScreen(std::format(L"Need 3 BodyInventory containers, found {}",
                    m_bodyInvHandles.size()), 3.0f, 1.0f, 0.3f, 0.3f);
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [Swap] FAIL: only {} BodyInventory containers (need 3)\n"),
                    m_bodyInvHandles.size());
                return;
            }
            ...
            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] toolbar swap: T{} -> T{} (stash->container[{}], restore<-container[{}])\n"),
                curTB + 1, nextTB + 1, curTB + 1, nextTB + 1);
            ...
            showOnScreen(std::format(L"Swapping to Toolbar {}...", nextTB + 1),
                         2.0f, 0.0f, 1.0f, 0.5f);
          } catch (const std::exception&) {
              Output::send<LogLevel::Warning>(
                  STR("[MoriaCppMod] [Swap] EXCEPTION in swapToolbar()\n"));
          } catch (...) {
              Output::send<LogLevel::Warning>(
                  STR("[MoriaCppMod] [Swap] UNKNOWN EXCEPTION in swapToolbar()\n"));
          }
        }
        ...
        void swapToolbarTick()
        {
            ...
            if (m_swap.wait > 0) { m_swap.wait--; return; }
            ...
            if (!playerChar) { m_swap.active = false; return; }
            auto* invComp = findPlayerInventoryComponent(playerChar);
            if (!invComp) { m_swap.active = false; return; }
            ...
            if (!getSlotFunc || !moveFunc) {
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] Swap: functions missing (GetItemForHotbarSlot={} MoveItem={})\n"),
                    getSlotFunc != nullptr, moveFunc != nullptr);
                m_swap.active = false;
                return;
            }
            ...
            if (!m_swap.resolved) {
                ...
                if (!m_dropItemMgr || !m_ihfCDO) {
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] Resolve: DropItemMgr={} IHF={} — using default mapping stash=[{}] restore=[{}]\n"),
                        m_dropItemMgr != nullptr, m_ihfCDO != nullptr, m_swap.stashIdx, m_swap.restoreIdx);
                    m_swap.wait = 1;
                } else {
                    ...
                    if (!getNameFunc || !ihfGetSlot || !ihfIsValid) {
                        Output::send<LogLevel::Warning>(
                            ...
                            getNameFunc != nullptr, ihfGetSlot != nullptr, ihfIsValid != nullptr);
                        m_swap.wait = 1;
                    } else {
                        ...
                        for (auto ci : hotbarCIs) {
                            for (auto n : c1CIs) if (n == ci) { c1Match++; break; }
                            for (auto n : c2CIs) if (n == ci) { c2Match++; break; }
                        }

                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] Resolve: hotbar={} items, c[1]={} items ({} match), c[2]={} items ({} match)\n"),
                            hotbarCIs.size(), c1CIs.size(), c1Match, c2CIs.size(), c2Match);
                        ...
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] Resolve: RESULT stashIdx={} restoreIdx={}\n"),
            ...
            if (m_swap.phase == 0) {
                ...
                if (!m_swap.cleared) {
                    ...
                    if (emptyFunc) {
                        ...
                        if (emptyItem.offset >= 0) {
                            ...
                            Output::send<LogLevel::Warning>(
                                STR("[MoriaCppMod] Phase0: EmptyContainer([{}]) — clearing stale items before stash\n"),
                                m_swap.stashIdx);
                        }
                    }
                    m_swap.wait = 3;
                ...
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] Phase0 done: stashed {} items. Restoring from container[{}]\n"),
                    m_swap.moved, m_swap.restoreIdx);
            ...
            if (m_swap.phase == 1) {
                ...
                if (emptyFunc) {
                    ...
                    if (emptyItem.offset >= 0) {
                        std::vector<uint8_t> emptyBuf(std::max(emptyFunc->GetParmsSize() + 32, 64), 0);
                        std::memcpy(emptyBuf.data() + emptyItem.offset,
                                    restoreContainer.data(), handleSize);
                        invComp->ProcessEvent(emptyFunc, emptyBuf.data());
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] Phase1: EmptyContainer([{}]) — cleanup after restore\n"),
                            m_swap.restoreIdx);
                    }
                }
                ...
                std::wstring msg = std::format(L"Toolbar {} active ({} items moved)",
                    m_swap.nextTB + 1, m_swap.moved);
                showOnScreen(msg, 3.0f, 0.0f, 1.0f, 0.5f);
                Output::send<LogLevel::Warning>(STR("[MoriaCppMod] {}\n"), msg);
                m_swap.active = false;
                refreshActionBar();
        ...
        void loadQuickBuildSlots()
        {
            ...
            if (loaded > 0) {
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] Loaded {} quick-build slots from disk\n"), loaded);
                updateOverlayText();
            }
        }
        ...
        void saveKeybindings()
        {
            ...
            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] Saved {} keybindings to disk\n"), BIND_COUNT);
        }
        ...
        void loadKeybindings()
        {
            ...
            if (loaded > 0) {
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] Loaded {} keybindings from disk\n"), loaded);
            }
        }
        ...
        static uint8_t* findBulkDataPtr(uint8_t* mipBase, int64_t expectedSize)
        {
            ...
            for (int mipOff = 16; mipOff <= 256; mipOff += 8) {
                ...
                if (!looksLikeDXT5(candidatePtr, expectedSize)) {
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] [Icon]   mip+{}: ptr 0x{:016X} readable but NOT DXT5\n"),
                        mipOff, candidate);
                    continue;
                }
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [Icon]   mip+{}: VALID DXT5 data at 0x{:016X}\n"),
                    mipOff, candidate);
                return candidatePtr;
            }
            return nullptr;
        ...
        static FProperty* findParam(UFunction* fn, const wchar_t* name)
        {
            ...
            for (auto* prop : fn->ForEachProperty()) {
                ...
                for (size_t i = 0; i < pn.size(); i++) {
                    if (towlower(pn[i]) != towlower(target[i])) { match = false; break; }
                }
        ...
        bool extractAndSaveIcon(UObject* widget, const std::wstring& textureName, const std::wstring& outPath)
        {
            ...
            try {
                ...
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [Icon] UTexture2D: {} '{}'\n"),
                    safeClassName(texture), std::wstring(texture->GetName()));

                // --- Find required UFunctions ---
                auto* createRTFn = UObjectGlobals::StaticFindObject<UFunction*>(nullptr, nullptr,
                    STR("/Script/Engine.KismetRenderingLibrary:CreateRenderTarget2D"));
                auto* beginDrawFn = UObjectGlobals::StaticFindObject<UFunction*>(nullptr, nullptr,
                    STR("/Script/Engine.KismetRenderingLibrary:BeginDrawCanvasToRenderTarget"));
                auto* endDrawFn = UObjectGlobals::StaticFindObject<UFunction*>(nullptr, nullptr,
                    STR("/Script/Engine.KismetRenderingLibrary:EndDrawCanvasToRenderTarget"));
                auto* exportRTFn = UObjectGlobals::StaticFindObject<UFunction*>(nullptr, nullptr,
                    STR("/Script/Engine.KismetRenderingLibrary:ExportRenderTarget"));
                auto* drawTexFn = UObjectGlobals::StaticFindObject<UFunction*>(nullptr, nullptr,
                    STR("/Script/Engine.Canvas:K2_DrawTexture"));

                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [Icon] CreateRT={} BeginDraw={} EndDraw={} ExportRT={} K2_DrawTex={}\n"),
                ...
                if (!s_loggedParams) {
                    ...
                    for (auto* fn : {createRTFn, beginDrawFn, endDrawFn, drawTexFn, exportRTFn}) {
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [Icon] {} ParmsSize={}:\n"),
                            std::wstring(fn->GetName()), fn->GetParmsSize());
                        for (auto* prop : fn->ForEachProperty()) {
                            Output::send<LogLevel::Warning>(
                                STR("[MoriaCppMod] [Icon]   {} @{} sz={} {}\n"),
                                std::wstring(prop->GetName()), prop->GetOffset_Internal(),
                                prop->GetSize(), std::wstring(prop->GetClass().GetName()));
                        }
                    }
                }
                ...
                UObject* worldCtx = nullptr;
                {
                    std::vector<UObject*> pcs;
                    UObjectGlobals::FindAllOf(STR("PlayerController"), pcs);
                    for (auto* pc : pcs) { if (pc) { worldCtx = pc; break; } }
                }
                if (!worldCtx) {
                ...
                auto* krlClass = UObjectGlobals::StaticFindObject<UClass*>(nullptr, nullptr,
                ...
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [Icon] Got Canvas: {} '{}'\n"),
                ...
                // === Step 3: K2_DrawTexture on Canvas ===
                {
                    ...
                    if (dPos) { auto* v = reinterpret_cast<float*>(dtParams.data() + dPos->GetOffset_Internal()); v[0] = 0.0f; v[1] = 0.0f; }
                    if (dSize) { auto* v = reinterpret_cast<float*>(dtParams.data() + dSize->GetOffset_Internal()); v[0] = 128.0f; v[1] = 128.0f; }
                    if (dCoordPos) { auto* v = reinterpret_cast<float*>(dtParams.data() + dCoordPos->GetOffset_Internal()); v[0] = 0.0f; v[1] = 0.0f; }
                    if (dCoordSize) { auto* v = reinterpret_cast<float*>(dtParams.data() + dCoordSize->GetOffset_Internal()); v[0] = 1.0f; v[1] = 1.0f; }
                    if (dColor) { auto* c = reinterpret_cast<float*>(dtParams.data() + dColor->GetOffset_Internal()); c[0] = 1.0f; c[1] = 1.0f; c[2] = 1.0f; c[3] = 1.0f; }
                    ...
                    if (dPivot) { auto* v = reinterpret_cast<float*>(dtParams.data() + dPivot->GetOffset_Internal()); v[0] = 0.5f; v[1] = 0.5f; }
                ...
                // === Step 4: EndDrawCanvasToRenderTarget ===
                {
                    ...
                    if (eCtx) {
                        ...
                        if (bCtx && bCtx->GetSize() <= eCtx->GetSize()) {
                            memcpy(eParams.data() + eCtx->GetOffset_Internal(),
                                   beginParams.data() + bCtx->GetOffset_Internal(),
                                   bCtx->GetSize());
                        }
                    }
                ...
                // === Step 5: ExportRenderTarget ===
                {
                    ...
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] [Icon] Exporting to folder='{}' file='{}'\n"), folder, fileName);
                ...
                for (const wchar_t* ext : {L".hdr", L".bmp", L".png", L".exr", L""}) {
                    ...
                    if (attr != INVALID_FILE_ATTRIBUTES) {
                        ...
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [Icon] Found file: {} ({} bytes)\n"), candidate, fsize);
                        if (fsize > 0) { exportedPath = candidate; break; }
                    }
                }
                            ...
                            for (UINT i = 0; i < num; i++) {
                                if (wcscmp(encoders[i].MimeType, L"image/png") == 0) {
                                    if (img->Save(outPath.c_str(), &encoders[i].Clsid, nullptr) == Gdiplus::Ok) {
                                        Output::send<LogLevel::Warning>(
                                            STR("[MoriaCppMod] [Icon] Converted to PNG: {}\n"), outPath);
                                        delete img;
        ...
        void assignRecipeSlot(int slot)
        {
                ...
                if (!m_recipeSlots[slot].textureName.empty()) {
                    ...
                    if (!s_overlay.iconFolder.empty()) {
                        ...
                        if (attr == INVALID_FILE_ATTRIBUTES) {
                            ...
                            Output::send<LogLevel::Warning>(
                                STR("[MoriaCppMod] [Icon] PNG already exists: {}\n"), pngPath);
                        }
                    }
                }
            }
            ...
            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] [QuickBuild] ASSIGN F{} = '{}'\n"), slot + 1, m_lastCapturedName);
        ...
        void dumpObjectProperties(UObject* obj, const std::wstring& label, int maxProps = 200)
        {
            ...
            for (auto* prop : obj->GetClassPrivate()->ForEachPropertyInChain()) {
                ...
                if (size == 8 && typeName.find(STR("Object")) != std::wstring::npos) {
                    ...
                    if (ptr && isReadableMemory(ptr, 64)) {
                        try {
                            valueInfo = L" -> " + safeClassName(ptr)
                                      + L" '" + std::wstring(ptr->GetName()) + L"'";
                        } catch (...) { valueInfo = L" -> (err)"; }
                    } else {
                    ...
                    wchar_t buf[32]{}; swprintf(buf, 32, L" = %.3f", *reinterpret_cast<float*>(base + offset));
                    valueInfo = buf;
                }
        ...
        void probeImageWidget(UObject* imageWidget, const std::wstring& name)
        {
            ...
            for (int row = 0; row < BRUSH_SZ; row += 16) {
                ...
                for (int col = 0; col < 16 && (row + col) < BRUSH_SZ; col++) {
                    wchar_t h[4]{}; swprintf(h, 4, L"%02X ", imgBase[BRUSH_OFF + row + col]);
                    hex += h;
                }
                ...
                if (val >= 0x10000 && val < 0x00007FF000000000) {
                    ...
                    if (isReadableMemory(ptr, 64)) {
                        try {
                            ...
                            Output::send<LogLevel::Warning>(
                                STR("[MoriaCppMod]   brush+72: {} '{}' @ 0x{:016X}\n"),
                                objCls, objName, val);
                        } catch (...) {
            ...
            const wchar_t* funcNames[] = {
                STR("SetBrushFromTexture"), STR("SetBrushResourceObject"),
                STR("SetBrush"), STR("SetBrushFromMaterial"),
                STR("SetBrushFromAtlasInterface"), STR("SetBrushFromSoftTexture"),
                STR("SetColorAndOpacity"), STR("SetOpacity"),
                STR("GetBrush"), STR("SetBrushSize"),
                STR("SetBrushTintColor"), STR("SetDesiredSizeOverride"),
            };
        ...
        void probeSelectedRecipe()
        {
            ...
            for (auto* w : widgets) {
                ...
                if (cls != L"UI_WBP_Build_Item_Medium_C") continue;
                std::wstring name = readWidgetDisplayName(w);
                if (name == m_lastCapturedName) { widget = w; break; }
            }
            if (!widget) {
                for (auto* w : widgets) {
                    if (!w) continue;
            ...
            if (iconImage && isReadableMemory(iconImage, 64)) {
                ...
                Output::send<LogLevel::Warning>(STR("[MoriaCppMod] Icon widget: {} '{}' @ {:p}\n"),
                    iconCls, std::wstring(iconImage->GetName()), static_cast<void*>(iconImage));
                    ...
                    try {
                        ...
                        if (getTexParam) {
                            ...
                            const wchar_t* texParamNames[] = {
                                STR("Texture"), STR("BaseColor"), STR("Icon"),
                                STR("DiffuseTexture"), STR("BaseColorTexture"),
                                STR("MainTexture"), STR("Image"), STR("T_Icon"),
                                STR("Param"), STR("TextureParam"),
                            };
                            for (auto* paramName : texParamNames) {
                                ...
                                if (args.ReturnValue && args.OutValue) {
                                    ...
                                    Output::send<LogLevel::Warning>(
                                        STR("[MoriaCppMod] >>> GetTextureParameterValue('{}') = {} '{}' <<<\n"),
                        ...
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] TextureParameterValues TArray: Data={:p} Num={}\n"),
                        ...
                        if (arrNum > 0 && arrNum < 32 && arrData && isReadableMemory(arrData, arrNum * 40)) {
                            ...
                            for (int entrySz : ENTRY_SIZES) {
                                Output::send<LogLevel::Warning>(
                                    STR("[MoriaCppMod] Trying entry size {}B:\n"), entrySz);
                                bool foundAny = false;
                                ...
                                for (int i = 0; i < arrNum && i < 8; i++) {
                                    ...
                                    for (int b = 0; b < entrySz && b < 56; b++) {
                                        wchar_t h[4]{}; swprintf(h, 4, L"%02X ", entry[b]);
                                        hex += h;
                                    }
                                    Output::send<LogLevel::Warning>(
                                        STR("[MoriaCppMod]   entry[{}]: {}\n"), i, hex);
                                        ...
                                        try {
                                            ...
                                            for (int toff = 8; toff < 800; toff += 8) {
                                                ...
                                                if (sx > 0 && sx <= 4096 && sy > 0 && sy <= 4096) {
                                                    Output::send<LogLevel::Warning>(
                                                        STR("[MoriaCppMod]   tex+{}: PlatformData? {}x{} @ 0x{:016X}\n"),
                                                        toff, sx, sy, tval);
                                                    // Dump first 80 bytes of PlatformData
                                                    std::wstring pdHex;
                                                    for (int pb = 0; pb < 80; pb++) {
                                                        wchar_t h[4]{}; swprintf(h, 4, L"%02X ", candidate[pb]);
                                                        pdHex += h;
                                                        if ((pb + 1) % 16 == 0) {
                                                            Output::send<LogLevel::Warning>(
                                                                STR("[MoriaCppMod]     +{:3d}: {}\n"), pb - 15, pdHex);
                                                    ...
                                                    for (int mipOff : { 16, 20, 24, 32 }) {
                                                        ...
                                                        if (mipNum > 0 && mipNum <= 16 && mipArrData
                                                            && isReadableMemory(mipArrData, mipNum * 8)) {
                                                            Output::send<LogLevel::Warning>(
                                                                STR("[MoriaCppMod]     PD+{}: Mips? Num={} Data={:p}\n"),
                                                                mipOff, mipNum, static_cast<void*>(mipArrData));
                                                            ...
                                                            if (mip0 && isReadableMemory(mip0, 64)) {
                                                                ...
                                                                Output::send<LogLevel::Warning>(
                                                                    STR("[MoriaCppMod]       Mip0: {}x{} @ {:p}\n"),
                                                                    m0sx, m0sy, static_cast<void*>(mip0));
                                                                // Hex dump first 128 bytes of mip0
                                                                if (isReadableMemory(mip0, 128)) {
                                                                    for (int mr = 0; mr < 128; mr += 16) {
                                                                        std::wstring mhex;
                                                                        for (int mc = 0; mc < 16; mc++) {
                                                                            wchar_t h[4]{}; swprintf(h, 4, L"%02X ", mip0[mr + mc]);
                                                                            mhex += h;
                                                                        }
                                                                        Output::send<LogLevel::Warning>(
                                                                            STR("[MoriaCppMod]         +{:3d}: {}\n"), mr, mhex);
            ...
            if (blockName && isReadableMemory(blockName, 64)) {
                ...
                if (getTextFn && getTextFn->GetParmsSize() == sizeof(FText)) {
                    ...
                    if (txt.Data)
                        Output::send<LogLevel::Warning>(STR("[MoriaCppMod] blockName text = \"{}\"\n"), txt.ToString());
                }
            }
            ...
            if (stackCount && isReadableMemory(stackCount, 64)) {
                ...
                if (getTextFn && getTextFn->GetParmsSize() == sizeof(FText)) {
                    ...
                    if (txt.Data)
                        Output::send<LogLevel::Warning>(STR("[MoriaCppMod] StackCount text = \"{}\"\n"), txt.ToString());
                }
            }
        ...
        void selectRecipeOnBuildTab(UObject* buildTab, int slot)
        {
            ...
            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] [QuickBuild] SELECT: searching for '{}' (slot F{})\n"),
                targetName, slot + 1);
            ...
            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] [QuickBuild]   checked {} visible widgets, match: {}\n"),
                visibleCount, matchedWidget ? L"YES" : L"NO");

            if (!matchedWidget) {
                showOnScreen((L"Recipe '" + targetName + L"' not found in menu!").c_str(),
                            3.0f, 1.0f, 0.3f, 0.0f);
                return;
            }
            ...
            if (m_bLockWidgetOffset >= 0) {
                ...
                if (m_bLockIsIndirect) {
                    ...
                    if (ptr && isReadableMemory(ptr, BLOCK_DATA_SIZE)) {
                        ...
                        gotFreshBLock = true;
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [QuickBuild]   using FRESH bLock from widget (indirect @{})\n"),
                            m_bLockWidgetOffset);
                    }
                } else {
                    ...
                    gotFreshBLock = true;
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] [QuickBuild]   using FRESH bLock from widget (direct @{})\n"),
                        m_bLockWidgetOffset);
                }
            }
            ...
            if (!gotFreshBLock && m_recipeSlots[slot].hasBLockData) {
                ...
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [QuickBuild]   using SAVED bLock (may be stale)\n"));
            } else if (!gotFreshBLock) {
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [QuickBuild]   WARNING: no bLock data at all, using zeros\n"));
            }
            ...
            *reinterpret_cast<int32_t*>(params + 128) = 0;
            ...
            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] [QuickBuild]   calling blockSelectedEvent with selfRef={:p}\n"),
        ...
        void quickBuildSlot(int slot)
        {
            ...
            if (!m_recipeSlots[slot].used) {
                std::wstring msg = L"F" + std::to_wstring(slot + 1) +
                    L" empty \x2014 select recipe in B menu, then Shift+F" + std::to_wstring(slot + 1);
            ...
            if (m_pendingQuickBuildSlot >= 0) {
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [QuickBuild] F{} pressed but slot {} already pending (frame {}), ignoring\n"),
                    slot + 1, m_pendingQuickBuildSlot + 1, m_pendingBuildFrames);
                return;
            }

            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] [QuickBuild] ACTIVATE F{} -> '{}' (charLoaded={} frameCounter={})\n"),
                slot + 1, m_recipeSlots[slot].displayName, m_characterLoaded, m_frameCounter);
            ...
            if (buildTab) {
                ...
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [QuickBuild] Build tab already open — closing first (pendingFrames=-15)\n"));
                ...
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [QuickBuild] Build tab not open — sending B key (pendingFrames=0)\n"));
                keybd_event(0x42, 0, 0, 0);
                keybd_event(0x42, 0, KEYEVENTF_KEYUP, 0);
                m_pendingQuickBuildSlot = slot;
        ...
        void buildFromTarget()
        {
            Output::send<LogLevel::Warning>(
                ...
                m_lastTargetBuildable, m_targetBuildName, m_targetBuildRecipeRef, m_characterLoaded, m_frameCounter);

            if (m_pendingTargetBuild) {
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [TargetBuild] Already pending (frame {}), ignoring\n"),
                    m_pendingTargetBuildFrames);
                return;
            }

            if (!m_lastTargetBuildable || (m_targetBuildName.empty() && m_targetBuildRecipeRef.empty())) {
                showOnScreen(L"No buildable target \x2014 aim at a building and press F10 first",
                            3.0f, 1.0f, 0.5f, 0.0f);
                return;
            }
            ...
            if (buildTab) {
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [TargetBuild] Build tab already open — closing first\n"));
                keybd_event(0x42, 0, 0, 0);
                keybd_event(0x42, 0, KEYEVENTF_KEYUP, 0);
                ...
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [TargetBuild] Build tab not open — sending B key\n"));
                keybd_event(0x42, 0, 0, 0);
                keybd_event(0x42, 0, KEYEVENTF_KEYUP, 0);
                m_pendingTargetBuild = true;
        ...
        {
            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] [TargetBuild] Searching: name='{}' recipeRef='{}' bLockOffset={} indirect={}\n"),
                m_targetBuildName, m_targetBuildRecipeRef, m_bLockWidgetOffset, m_bLockIsIndirect);
            ...
            std::vector<std::pair<std::wstring, uint32_t>> targetFNames;
            {
                ...
                if (noBP.size() > 3 && noBP.substr(0, 3) == L"BP_")
                    noBP = noBP.substr(3);
                ...
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [TargetBuild] FName CIs: full='{}' CI={}, short='{}' CI={}\n"),
                    m_targetBuildRecipeRef, ci1, noBP, ci2);
            }
            ...
            for (auto* w : widgets) {
                ...
                if (isFirstFew) {
                    std::wstring objName(w->GetName());
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] [TargetBuild]   W[{}] obj='{}' display='{}'\n"),
                ...
                if (!matchedWidget && m_bLockWidgetOffset >= 0 && !m_targetBuildRecipeRef.empty()) {
                    ...
                    if (!bLock) {
                        bLockNullCount++;
                        if (isFirstFew) Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [TargetBuild]     bLock=NULL\n"));
                    } else if (!isReadableMemory(bLock + 104, 16)) {
                        bLockMemFailCount++;
                        if (isFirstFew) Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [TargetBuild]     bLock+104 not readable\n"));
                    } else {
                        ...
                        if (isFirstFew) {
                            ...
                            Output::send<LogLevel::Warning>(
                                STR("[MoriaCppMod] [TargetBuild]     bLock tag CI={} Num={} | variants={} ptr={:p}\n"),
                                tagCI, tagNum, variantsCount, (void*)variantsPtr);
                        }
                        ...
                        if (variantsCount <= 0 || !variantsPtr) {
                            ...
                            if (isFirstFew) {
                                Output::send<LogLevel::Warning>(
                                    STR("[MoriaCppMod] [TargetBuild]     RowName CI={} Num={}\n"),
                                    rowCI, rowNum);
                            }
                            ...
                            for (auto& [tName, tCI] : targetFNames) {
                                if (tCI == rowCI) {
                                    ...
                                    Output::send<LogLevel::Warning>(
                                        STR("[MoriaCppMod] [TargetBuild] MATCH (bLock RowName CI={}) on '{}' target='{}'\n"),
                                        rowCI, matchedName, tName);
                                    break;
                                }
                            }
                        } else if (isFirstFew) {
                            Output::send<LogLevel::Warning>(
                                STR("[MoriaCppMod] [TargetBuild]     variantsPtr not readable (0xE8 bytes)\n"));
                        }
                    }
                }
                ...
                if (!matchedWidget && !name.empty() && name == m_targetBuildName) {
                    ...
                    matchedName = name;
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] [TargetBuild] MATCH (exact display name) on '{}'\n"), name);
                }
                ...
                if (!matchedWidget && !name.empty()) {
                    ...
                    if (refNoBP.size() > 2 && refNoBP.substr(0, 2) == L"bp")
                        refNoBP = refNoBP.substr(2);
                    ...
                    if (nameNorm == refNoBP || nameNorm == targetNorm) {
                        ...
                        matchedName = name;
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [TargetBuild] MATCH (normalized exact) on '{}'\n"), name);
                    }
                }
            ...
            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] [TargetBuild] Result: {} visible, bLockNull={} memFail={} varEmpty={} match={}\n"),
                visibleCount, bLockNullCount, bLockMemFailCount, variantsEmptyCount,
                matchedWidget ? matchedName.c_str() : L"NO");

            if (!matchedWidget) {
                showOnScreen((L"Recipe '" + m_targetBuildName + L"' not found in build menu").c_str(),
                            3.0f, 1.0f, 0.3f, 0.0f);
                return;
            }
            ...
            Output::send<LogLevel::Warning>(
                STR("[MoriaCppMod] [TargetBuild] Calling blockSelectedEvent: freshBLock={} selfRef={:p}\n"),
                gotFreshBLock, static_cast<void*>(matchedWidget));
        ...
        void updateOverlaySlots()
        {
            ...
            for (int i = 0; i < OVERLAY_BUILD_SLOTS && i < QUICK_BUILD_SLOTS; i++) {
                ...
                if (s_overlay.slots[i].used && !s_overlay.slots[i].textureName.empty()
                    && !s_overlay.slots[i].icon && !s_overlay.iconFolder.empty()) {
        ...
        void updateOverlayText() { updateOverlaySlots(); }
        ...
        void stopOverlay()
        {
            ...
            if (s_overlay.overlayHwnd)
                PostMessage(s_overlay.overlayHwnd, WM_CLOSE, 0, 0);
            if (s_overlay.thread) {
        ...
        void stopConfig()
        {
            ...
            if (s_config.configHwnd)
                PostMessage(s_config.configHwnd, WM_CLOSE, 0, 0);
            if (s_config.thread) {
        ...
        void stopTargetInfo()
        {
            ...
            if (s_targetInfo.hwnd)
                PostMessage(s_targetInfo.hwnd, WM_CLOSE, 0, 0);
        ...
        void showTargetInfo(const std::wstring& name, const std::wstring& display,
                           const std::wstring& path, const std::wstring& cls,
                           bool buildable = false, const std::wstring& recipe = L"",
    ...
    public:
        ...
        auto on_unreal_init() -> void override
        {
            ...
            // Inventory probe (Num.)
            register_keydown_event(Input::Key::DECIMAL, [this]() { probeInventoryHotbar(); });

            // Clear hotbar (Alt+Num.) — move hotbar items to inventory or drop
            register_keydown_event(Input::Key::DECIMAL, {Input::ModifierKey::ALT}, [this]() { clearHotbar(); });
            ...
            register_keydown_event(Input::Key::INS, [this]() {
                ...
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [SpyAll] === ALL ProcessEvent logging for 3s — press B NOW! ===\n"));
                showOnScreen(L"SPY ALL ACTIVE (3s) \x2014 press B now!", 3.0f, 1.0f, 1.0f, 0.0f);
            });

            // Deep probe: dump all data on selected build recipe
            register_keydown_event(Input::Key::DIVIDE, [this]() { probeSelectedRecipe(); });
            ...
            const Input::Key fkeys[] = {
                Input::Key::F1,  Input::Key::F2,  Input::Key::F3,  Input::Key::F4,
                Input::Key::F5,  Input::Key::F6,  Input::Key::F7,  Input::Key::F8
            };
            for (int i = 0; i < 8; i++) {  // F1-F8 for quickbuild
                register_keydown_event(fkeys[i], [this, i]() { quickBuildSlot(i); });
                register_keydown_event(fkeys[i], {Input::ModifierKey::SHIFT}, [this, i]() { assignRecipeSlot(i); });
            }

            // \ (backslash): toggle between 2 toolbars
            register_keydown_event(Input::Key::OEM_FIVE, [this]() { swapToolbar(); });

            // F12: toggle config window
            register_keydown_event(Input::Key::F12, [this]() { toggleConfig(); });
            ...
            register_keydown_event(Input::Key::F10, [this]() { dumpAimedActor(); });

            // Shift+F10: Build the last targeted buildable object
            register_keydown_event(Input::Key::F10, {Input::ModifierKey::SHIFT}, [this]() { buildFromTarget(); });
            ...
            register_keydown_event(Input::Key::MULTIPLY, [this]() {
                ...
                s_overlay.visible = m_showHotbar;
                s_overlay.needsUpdate = true;
                showOnScreen(m_showHotbar ? L"Hotbar overlay ON" : L"Hotbar overlay OFF",
                            2.0f, 0.2f, 0.8f, 1.0f);
            });
            ...
            Unreal::Hook::RegisterProcessEventPostCallback([](UObject* context, UFunction* func, void* parms) {
                ...
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [QuickBuild] Captured: '{}' (with bLock data)\n"), displayName);
                ...
                if (s_instance->m_bLockWidgetOffset < 0) {
                    ...
                    for (int off = 0; off <= 2048 - BLOCK_DATA_SIZE; off += 8) {
                        if (std::memcmp(widgetBase + off, p, 16) == 0) {
                            ...
                            bool match = (std::memcmp(widgetBase + off, p, 104) == 0) &&
                                         (std::memcmp(widgetBase + off + 112, p + 112, 8) == 0);
                            if (match) {
                                s_instance->m_bLockWidgetOffset = off;
                                Output::send<LogLevel::Warning>(
                                    STR("[MoriaCppMod] [QuickBuild] DISCOVERED: bLock data at widget offset {} (0x{:X})\n"),
                                    off, off);
                                break;
                            }
                        }
                    ...
                    if (s_instance->m_bLockWidgetOffset < 0) {
                        ...
                        for (int off = 0; off <= 2048 - 8; off += 8) {
                            ...
                            if (std::memcmp(ptr, p, 104) == 0 &&
                                std::memcmp(ptr + 112, p + 112, 8) == 0) {
                                // Found it via pointer indirection
                                s_instance->m_bLockWidgetOffset = off;
                                s_instance->m_bLockIsIndirect = true;
                                Output::send<LogLevel::Warning>(
                                    STR("[MoriaCppMod] [QuickBuild] DISCOVERED: bLock via POINTER at widget offset {} (0x{:X})\n"),
                                    off, off);
                                break;
                            }
                        }
                    ...
                    if (s_instance->m_bLockWidgetOffset < 0) {
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [QuickBuild] WARNING: bLock data NOT found in widget memory!\n"));
                    }
                }
            });
        ...
        auto on_update() -> void override
        {
            ...
            if (m_pendingQuickBuildSlot >= 0) {
                ...
                if (m_pendingBuildFrames == 0) {
                    ...
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] [QuickBuild] SM: frame 0 — sending B key to open menu\n"));
                    keybd_event(0x42, 0, 0, 0);
                    keybd_event(0x42, 0, KEYEVENTF_KEYUP, 0);
                    ...
                    if (!buildTab) {
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [QuickBuild] SM: frame 45 — tab NOT visible, RETRYING B key\n"));
                        keybd_event(0x42, 0, 0, 0);
                        keybd_event(0x42, 0, KEYEVENTF_KEYUP, 0);
                    }
                } else if (m_pendingBuildFrames > 150) {
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] [QuickBuild] SM: TIMEOUT at frame {} — build tab never became visible\n"),
                        m_pendingBuildFrames);
                    ...
                    if (buildTab) {
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [QuickBuild] SM: build tab found at frame {} — selecting recipe\n"),
                            m_pendingBuildFrames);
            ...
            // This polls the ACTUAL bound key (from s_bindings[11]) as a functional fallback.
            {
                ...
                if (nowDown && !s_lastSwapKey) {
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] [Swap] Key 0x{:02X} detected via GetAsyncKeyState fallback\n"),
                        swapVK);
                    swapToolbar();
                }
            ...
            if (m_pendingTargetBuild) {
                ...
                if (m_pendingTargetBuildFrames == 0) {
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] [TargetBuild] SM: frame 0 — sending B key to open menu\n"));
                    keybd_event(0x42, 0, 0, 0);
                    keybd_event(0x42, 0, KEYEVENTF_KEYUP, 0);
                    ...
                    if (!buildTab) {
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [TargetBuild] SM: frame 45 — tab NOT visible, RETRYING B key\n"));
                        keybd_event(0x42, 0, 0, 0);
                        keybd_event(0x42, 0, KEYEVENTF_KEYUP, 0);
                    }
                } else if (m_pendingTargetBuildFrames > 150) {
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] [TargetBuild] SM: TIMEOUT at frame {} — build tab never became visible\n"),
                        m_pendingTargetBuildFrames);
                    showOnScreen(L"Build menu didn't open (timeout)", 3.0f, 1.0f, 0.3f, 0.0f);
                    ...
                    if (buildTab) {
                        Output::send<LogLevel::Warning>(
                            STR("[MoriaCppMod] [TargetBuild] SM: build tab found at frame {} — selecting recipe\n"),
                            m_pendingTargetBuildFrames);
            ...
            if (m_characterLoaded && m_frameCounter % 60 == 0) {
                ...
                if (dwarves.empty()) {
                    ...
                    s_targetInfo.visible = false;
                    Output::send<LogLevel::Warning>(
                        STR("[MoriaCppMod] [Swap] Cleared all container handles, swap state, and cheat toggles\n"));
                }
            }
            ...
            if (m_bodyInvHandle.empty() && framesSinceChar > 300 && framesSinceChar < 3900 && framesSinceChar % 120 == 0) {
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [Swap] Container scan attempt (frame {}). bodyInvHandle.empty={} handles.size={}\n"),
                    m_frameCounter, m_bodyInvHandle.empty(), m_bodyInvHandles.size());
                UObject* playerChar = nullptr;
                {
                    std::vector<UObject*> actors;
                    UObjectGlobals::FindAllOf(STR("Character"), actors);
            ...
            if (m_bodyInvHandle.empty() && framesSinceChar == 3900) {
                Output::send<LogLevel::Warning>(
                    STR("[MoriaCppMod] [Swap] Container discovery FAILED after 60s — toolbar swap unavailable this session\n"));
