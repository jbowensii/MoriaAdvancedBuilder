// MoriaCppMod v6.23.1 - Return to Moria UE4SS C++ mod (~15,500 lines after v6.23.1 Pass 2 cleanup)
...
namespace MoriaMods
{
    ...
    class MoriaCppMod : public RC::CppUserModBase
    {
      private:
        ...
        // v6.23.1 - The controller cluster (~720 LOC: DualSenseReader +
        ...
        // v6.21.36 - dedicated UserWidget hosting our standalone EditableTextBox
        ...
        // v6.21.36 - watchdog flag: hovering the popup's Confirm/Cancel
        ...
        // v6.21.36 - HUD reposition mode (BIND_REPOSITION_HUD, default F10).
        ...
        // v6.21.36 - per-cell key marker TextBlocks. Refreshed each tick from
      ...
      public:
        ...
        MoriaCppMod()
        {
            ...
            VLOG(STR("[MoriaCppMod] Loaded v6.23.1\n"));
        }
        ...
        auto on_unreal_init() -> void override
        {
            ...
            loadConfig();
            VLOG(STR("[MoriaCppMod] Loaded v6.23.1 (workDir={})\n"),
            ...
            Unreal::Hook::RegisterProcessEventPreCallback([](UObject* context, UFunction* func, void* parms) {
                ...
                // v6.22.5+v6.23.1: appendCheatsTabToArray call removed from
                ...
                // (v6.22.4+v6.23.1: PE pre-hook gamepad suppression and
            ...
            m_replayActive = true;
            VLOG(
                    STR("[MoriaCppMod] v6.23.1: F1-F8=build | F9=rotate | F12=config | Num0=bubble info | Num*=reveal map | Mod keybinds in Settings → keymap tab\n"));
            ...
            Unreal::Hook::RegisterLoadMapPreCallback(
                [this](UEngine*, FWorldContext&, FURL, UPendingNetGame*, FString&) -> std::pair<bool, bool>
                {
                    ...
                    //  v6.23.1 along with the rest of the FGK runtime-injection
        ...
        // (v6.23.1: FGK runtime-injection diagnostics deleted -
        ...
        void gameThreadTick(float deltaSeconds)
        {
            ...
            // v6.21.38 - inspect window auto-hide was duplicated here AND
            ...
            // v6.21.36 - F12 (BIND_CONFIG = MC_BIND_BASE + 5) dispatcher
            ...
            // v6.21.36 - Save Game keybind dispatcher (BIND_SAVE_GAME, default F12).
            ...
            // v6.21.36 - Reposition HUD keybind dispatcher (default F10).
