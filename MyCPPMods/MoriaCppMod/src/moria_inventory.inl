...
{
    if (s_off_aieStride > 0 && s_off_aieOnItem >= 0 && s_off_aieEffect >= 0) return true;
                    ...
                    if (auto* listProp = fastArrStruct->GetPropertyByNameInChain(STR("List")))
                    {
                        if (auto* listArrProp = CastField<FArrayProperty>(listProp))
                        {
                            ...
                            if (auto* innerStructProp = CastField<FStructProperty>(innerProp))
                            {
                                ...
                                if (aieStruct)
                                {
                                    if (auto* sStruct = static_cast<UScriptStruct*>(aieStruct)) stride = static_cast<int>(sStruct->GetStructureSize());
                                    if (auto* onItemProp = aieStruct->GetPropertyByNameInChain(STR("OnItem"))) offOnItem = onItemProp->GetOffset_Internal();
...
bool removeItemEffectFromList(UObject* invComp, int32_t itemID, const wchar_t* effectClassName)
{
    ...
    for (int32_t j = arrNum - 1; j >= 0; j--)
    {
        ...
        if (j < arrNum - 1) std::memcpy(e, arrData + (arrNum - 1) * stride, stride);
        arrNum--;
        return true;
    }
    return false;
...
void dumpItemEffects(UClass* itemClass, int32_t itemID, UObject* invComp)
{
    ...
    if (effectsProp)
    {
        ...
        if (isReadableMemory(effectsBase, 16))
        {
            ...
            {
                uint8_t* e = arrData + j * stride;
                if (!isReadableMemory(e, stride))
                {
                    VLOG(STR("[MoriaCppMod] [EffectDump]   [{}] <unreadable>\n"), j);
                    continue;
                }
    ...
    for (auto* comp : allComps)
    {
        ...
        for (int32_t i = 0; i < arrNum; i++)
        {
            ...
            if (s_verbose)
            {
                ...
                RC::Output::send<RC::LogLevel::Warning>(STR("[MoriaCppMod] [InvAudit]   [{}] id={} slot={} count={} class={}{}\n"), i, id, sl, cnt, nm, extra);
            }
        }
                ...
                if (cClass)
                {
                    if (UObject* cdo = cClass->GetClassDefaultObject())
                    {
                        ...
                        if (auto** limitPtr = cdo->GetValuePtrByPropertyNameInChain<UObject*>(STR("Storage")))
                        {
                            ...
                            {
                                if (auto* ms = limit->GetValuePtrByPropertyNameInChain<int32_t>(STR("MaxSlots"))) cMax = *ms;
                            }
                        }
                    }
                }
                if (slot >= cStart && slot < cStart + cMax)
                {
                    inContainer = true;
                    break;
                }
...
void captureLastChangedItem(UObject* invComp, void* parms)
{
    ...
    for (int32_t i = 0; i < arrNum; i++)
    {
        ...
        if (entryClass)
        {
            ...
            if (m_lastPickedUpDisplayName.empty()) m_lastPickedUpDisplayName = m_lastPickedUpItemName;
...
void replenishLastItem()
{
    ...
    DataTableUtil* itemTables[] = {&m_dtItems, &m_dtWeapons, &m_dtTools, &m_dtArmor, &m_dtConsumables, &m_dtOres, &m_dtContainerItems};
    int32_t maxStack = 0;
...
void removeItemAttributes()
{
...

    {
        int32_t targetID = *reinterpret_cast<int32_t*>(m_lastItemHandle);
        UObject* invComp = findPlayerInventoryComponent(pawn);
        if (invComp && removeItemEffectFromList(invComp, targetID, STR("MorRuneEffect"))) removed++;
    }
...
void showTrashDialog()
{
    ...
    {
        VLOG(STR("[MoriaCppMod] [Trash] BLOCKED: already visible\n"));
        return;
    }
    if (!m_lastPickedUpItemClass)
    {
    ...
    if (!s_genericPopupCls)
    {
        try
        {
            s_genericPopupCls = UObjectGlobals::StaticFindObject<UClass*>(nullptr, nullptr, STR("/Game/UI/PopUp/WBP_UI_GenericPopup.WBP_UI_GenericPopup_C"));
        }
        catch (...)
        {
        }
    ...
    if (!popup)
    {
        VLOG(STR("[MoriaCppMod] [Trash] popup spawn failed\n"));
        return;
    }
        ...
        if (ihfClass)
        {
            ...
            if (ihfCDO)
            {
                ...
                if (isContFn)
                {
                    ...
                    if (safeProcessEvent(ihfCDO, isContFn, cbuf.data()) && pR) isContainer = *reinterpret_cast<bool*>(cbuf.data() + pR->GetOffset_Internal());
                }
            }
        }
    }
    ...
    if (auto* pc2 = findPlayerController()) m_trashCursorWasVisible = getBoolProp(pc2, L"bShowMouseCursor");
    ...
    if (!m_trashCursorWasVisible) setInputModeUI(popup);
...
void onTrashPopupButtonClicked(UObject* context)
{
    ...
    if (isConfirm)
        confirmTrashItem(); // confirms + hides
    else
        hideTrashDialog();
}
...
void hideTrashDialog()
{
    ...
    if (m_trashDlgWidget)
    {
        ...
        if (auto* hideFn = m_trashDlgWidget->GetFunctionByNameInChain(STR("Hide")))
        {
            ...
            try
            {
                safeProcessEvent(m_trashDlgWidget, hideFn, b.data());
            }
            catch (...)
            {
            }
    ...
    // before the popup.
    if (!m_trashCursorWasVisible) setInputModeGame();
...
void confirmTrashItem()
{
    ...
    for (int iter = 0; iter < MAX_DROP_ITERATIONS && remaining > 0; iter++)
    {
...

        {
            ...
            if (iter == 0) VLOG(STR("[MoriaCppMod] [Trash] DropItem(handle, count={}) for {}\n"), remaining, m_lastPickedUpItemName);
...

        {
            ...
            for (auto* actor : postDropActors)
            {
                ...
                if (destroyFn2)
                {
                    safeProcessEvent(actor, destroyFn2, nullptr);
                    totalDestroyed++;
                }
            }
        }
                ...
                if (isReadableMemory(lb3, 16))
                {
                    ...
                    if (ad3 && an3 > 0 && an3 < 10000)
                    {
                        ...
                        for (int32_t i = 0; i < an3; i++)
                        {
                            ...
                            {
                                nowCount = *reinterpret_cast<int32_t*>(e3 + iiCountOff());
                                break;
                            }
                        }
                    }
