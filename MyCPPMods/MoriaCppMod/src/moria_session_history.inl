...
static inline const uint8_t* sessionPwdKey()
{
    ...
    static const uint8_t k[16] = {0x4D, 0x6F, 0x72, 0x69, 0x61, 0x4D, 0x6F, 0x64, 0x53, 0x65, 0x73, 0x73, 0x4B, 0x65, 0x79, 0x21};
    return k;
}
...
std::string serializeSessionHistoryJson()
{
    ...
    for (size_t i = 0; i < m_sessionHistory.size(); ++i)
    {
        ...
        out += ", ";
        out += "\"domain\": ";
        jsonEscape(out, e.domain);
        out += ", ";
        out += "\"port\": ";
        jsonEscape(out, e.port);
        out += ", ";
        out += "\"password\": ";
        jsonEscape(out, obfuscatePassword(e.password));
        out += ", ";
        out += "\"lastJoined\": ";
        jsonEscape(out, e.lastJoined);
        out += "}";
...
bool parseSessionHistoryJson(const std::string& text, std::vector<SessionHistoryEntry>& out)
{
    out.clear();
    ...
    while (c.p < c.end)
    {
        ...
        while (true)
        {
            ...
            if (key == "name")
                ...
                e.password = deobfuscatePassword(val);
            else if (key == "lastJoined")
                e.lastJoined = val;
...
void loadSessionHistory()
{
    ...
    {
        VLOG(STR("[SessionHistory] loaded {} entries from '{}'\n"), (int)m_sessionHistory.size(), utf8ToWide(path).c_str());
    }
    m_sessionHistoryLoaded = true;
}
...
static bool sessionEntryMatches(const SessionHistoryEntry& e, const std::string& domain, const std::string& port)
{
...
#endif
    char buf[32];
    std::snprintf(buf, sizeof(buf), "%04d-%02d-%02dT%02d:%02d:%02dZ", gm.tm_year + 1900, gm.tm_mon + 1, gm.tm_mday, gm.tm_hour, gm.tm_min, gm.tm_sec);
    return buf;
}
...
void addOrUpdateSessionHistory(const SessionHistoryEntry& incoming)
{
    ...
    for (auto& e : m_sessionHistory)
    {
        if (sessionEntryMatches(e, incoming.domain, incoming.port))
        {
            ...
            e.lastJoined = incoming.lastJoined.empty() ? nowIsoTimestamp() : incoming.lastJoined;
            saveSessionHistory();
            return;
        }
    }
...
bool removeSessionHistory(const std::string& domain, const std::string& port)
{
    if (!m_sessionHistoryLoaded) loadSessionHistory();
...
UClass* resolveSessionHistoryItemClass(UObject* sessionBox)
{
    ...
    if (!m_jwCls_SessionHistoryItem)
    {
        try
        {
            m_jwCls_SessionHistoryItem =
                    UObjectGlobals::StaticFindObject<UClass*>(nullptr,
                                                              nullptr,
...
void setTextBlockTextByName(UObject* userWidget, const wchar_t* childName, const std::wstring& text)
{
...
{
    if (s_off_chiStride > 0 && s_off_chiWorldName >= 0 && s_off_chiConnType >= 0 && s_off_chiInviteString >= 0 && s_off_chiUniqueInvite >= 0 &&
        s_off_chiPassword >= 0 && s_off_chiIsDedicated >= 0 && s_off_chiCreated >= 0)
        return true;
    ...
    if (auto* sProp = CastField<FStructProperty>(pData))
    {
        if (UStruct* sStruct = sProp->GetStruct())
        {
            ...
            if (auto* p = sStruct->GetPropertyByNameInChain(STR("WorldName"))) offWorldName = p->GetOffset_Internal();
            if (auto* p = sStruct->GetPropertyByNameInChain(STR("ConnectionType"))) offConnType = p->GetOffset_Internal();
            if (auto* p = sStruct->GetPropertyByNameInChain(STR("InviteString"))) offInviteString = p->GetOffset_Internal();
            if (auto* p = sStruct->GetPropertyByNameInChain(STR("UniqueInviteCode"))) offUniqueInvite = p->GetOffset_Internal();
            if (auto* p = sStruct->GetPropertyByNameInChain(STR("OptionalPassword"))) offPassword = p->GetOffset_Internal();
            if (auto* p = sStruct->GetPropertyByNameInChain(STR("bIsDedicatedServer"))) offIsDedicated = p->GetOffset_Internal();
            if (auto* p = sStruct->GetPropertyByNameInChain(STR("CreationDateTime"))) offCreated = p->GetOffset_Internal();
        }
    }
    ...
    VLOG(STR("[SessionHistory] FMorConnectionHistoryItem layout resolved: stride={:#x} "
         ...
         (unsigned)s_off_chiStride,
         (unsigned)s_off_chiWorldName,
         (unsigned)s_off_chiConnType,
         (unsigned)s_off_chiInviteString,
         (unsigned)s_off_chiUniqueInvite,
         (unsigned)s_off_chiPassword,
         (unsigned)s_off_chiIsDedicated,
         (unsigned)s_off_chiCreated);
    return true;
}
...
void buildConnectionHistoryItem(uint8_t* dst, const SessionHistoryEntry& e, bool isDedicated)
{
...
void injectSessionHistoryRows(UObject* spawnedJoinWorld)
{
    ...
    for (size_t idx = 0; idx < m_sessionHistory.size(); ++idx)
    {
        ...
        buildConnectionHistoryItem(buf.data() + pData->GetOffset_Internal(), e, /*isDedicated*/ true);
        try
        {
...
void tickSessionHistoryDeferredCapture()
{
    ...
    auto readFieldText = [&](const wchar_t* childName) -> std::string {
        ...
        try
        {
            safeProcessEvent(tb, getFn, buf.data());
        }
        catch (...)
        {
            return "";
        ...
        if (!pRet) return "";
        auto* asStrFn = UObjectGlobals::StaticFindObject<UFunction*>(nullptr, nullptr, STR("/Script/Engine.KismetTextLibrary:Conv_TextToString"));
        auto* ktlClass = UObjectGlobals::StaticFindObject<UClass*>(nullptr, nullptr, STR("/Script/Engine.KismetTextLibrary"));
        ...
        if (!pIn || !pOut) return "";
        std::memcpy(sb.data() + pIn->GetOffset_Internal(), buf.data() + pRet->GetOffset_Internal(), pIn->GetSize());
        try
        {
            safeProcessEvent(ktlCDO, asStrFn, sb.data());
        }
        catch (...)
        {
            return "";
    ...
    std::string hostOrPort = readFieldText(isLocal ? STR("TextField_LocalJoinPort") : STR("TextField_DirectJoinIP"));
    std::string pwd = readFieldText(isLocal ? STR("TextField_LocalJoinPassword") : STR("TextField_DirectJoinPassword"));
    ...
    else
    {
        ...
        if (colon != std::string::npos && colon > 0 && colon < hostOrPort.size() - 1)
        {
            ...
            if (allDigits)
            {
                domain = hostOrPort.substr(0, colon);
                port = tail;
            }
        }
    }
...
UClass* resolveGenericPopupClass()
{
    ...
    try
    {
        m_jwCls_GenericPopup = UObjectGlobals::StaticFindObject<UClass*>(nullptr, nullptr, STR("/Game/UI/PopUp/WBP_UI_GenericPopup.WBP_UI_GenericPopup_C"));
    }
    catch (...)
    {
    }
...
bool getWidgetSlateRect(UObject* widget, float& outX, float& outY, float& outW, float& outH)
{
    ...
    outH = sizeY;
    outX = 0;
    outY = 0; // unknown without full struct decode — caller falls back to index math
...
UObject* showDeleteConfirmPopup(size_t entryIndex)
{
    ...
    if (auto* showFn = popup->GetFunctionByNameInChain(STR("OnShowWithTwoButtons")))
    {
        ...
        if (entryIndex < m_sessionHistory.size())
        {
            ...
            entryLabel = utf8ToWide(e.name) + L" (" + utf8ToWide(e.domain) + L":" + utf8ToWide(e.port) + L")";
        }
...
void onAnyMenuButtonClicked(UObject* context, const wchar_t* fnName)
{
    ...
    VLOG(STR("[SessionHistory] popup button event fn='{}' -> {}\n"), fnName ? fnName : STR("?"), isConfirm ? STR("CONFIRM") : STR("CANCEL"));
