...
void spawnVfxAtLocation(UObject* worldContext, UObject* niagaraSystem, float x, float y, float z)
{
    static UFunction* s_fn = nullptr;
    static UObject* s_cdo = nullptr;
    if (!s_fn)
    {
        s_fn = UObjectGlobals::StaticFindObject<UFunction*>(nullptr, nullptr, STR("/Script/Niagara.NiagaraFunctionLibrary:SpawnSystemAtLocation"));
        s_cdo = UObjectGlobals::StaticFindObject<UObject*>(nullptr, nullptr, STR("/Script/Niagara.Default__NiagaraFunctionLibrary"));
    }
    ...
    if (s_parmsSize < 0)
    {
        ...
        for (auto* prop : s_fn->ForEachProperty())
        {
            ...
            if (name == L"WorldContextObject")
                s_offWCO = off;
            else if (name == L"SystemTemplate")
                s_offSystem = off;
            else if (name == L"Location")
                s_offLocation = off;
            else if (name == L"Rotation")
                s_offRotation = off;
            else if (name == L"Scale")
                s_offScale = off;
            else if (name == L"bAutoDestroy")
                s_offAutoDestroy = off;
            else if (name == L"bAutoActivate")
                s_offAutoActivate = off;
            else if (name == L"PoolingMethod")
                s_offPooling = off;
            else if (name == L"bPreCullCheck")
                s_offPreCull = off;
...
void spawnPointLightAtLocation(UObject* worldContext, float x, float y, float z, bool critical)
{
    ...
    {
        s_resolved = true;
        s_spawnFn = UObjectGlobals::StaticFindObject<UFunction*>(nullptr, nullptr, STR("/Script/Engine.GameplayStatics:BeginDeferredActorSpawnFromClass"));
        s_gsCDO = UObjectGlobals::StaticFindObject<UObject*>(nullptr, nullptr, STR("/Script/Engine.Default__GameplayStatics"));
        s_lightClass = UObjectGlobals::StaticFindObject<UClass*>(nullptr, nullptr, STR("/Script/Engine.PointLight"));
        s_finishFn = UObjectGlobals::StaticFindObject<UFunction*>(nullptr, nullptr, STR("/Script/Engine.GameplayStatics:FinishSpawningActor"));
        ...
        for (auto* prop : s_spawnFn->ForEachProperty())
        {
            ...
            if (name == L"WorldContextObject")
                s_sWCO = off;
            else if (name == L"ActorClass")
                s_sClass = off;
            else if (name == L"SpawnTransform")
                s_sXform = off;
            else if (name == L"ReturnValue")
                s_sRet = off;
        }
    ...
    if (s_sRet >= 0) spawned = *reinterpret_cast<UObject**>(buf.data() + s_sRet);
    ...
    if (s_finishFn && s_fActor >= 0)
    {
        ...
        if (s_fXform >= 0) std::memcpy(finBuf.data() + s_fXform, &xform, 48);
        ...
        if (!s_lightResolved)
        {
            ...
            if (s_setIntFn)
            {
                ...
                for (auto* p : s_setIntFn->ForEachProperty())
                {
                    ...
                    if (n == L"NewIntensity" || n == L"NewLightBrightness") s_intOff = p->GetOffset_Internal();
                }
            }
            if (s_setColorFn)
            {
                ...
                for (auto* p : s_setColorFn->ForEachProperty())
                {
                    if (!p) continue;
                    std::wstring n(p->GetName());
                    if (n == L"NewLightColor") s_colOff = p->GetOffset_Internal();
                }
            }
            if (s_setRadFn)
            {
                ...
                for (auto* p : s_setRadFn->ForEachProperty())
                {
                    if (!p) continue;
                    std::wstring n(p->GetName());
                    if (n == L"NewRadius") s_radOff = p->GetOffset_Internal();
                }
            }
...
void destroyAuditActors()
{
    for (auto& weakActor : m_auditSpawnedActors)
    {
        ...
        if (destroyFn) safeProcessEvent(actor, destroyFn, nullptr);
    }
    if (!m_auditSpawnedActors.empty()) VLOG(STR("[MoriaCppMod] [STAB] Destroyed {} spawned actor(s)\n"), m_auditSpawnedActors.size());
...
{
    destroyAuditActors();
    if (!m_auditLocations.empty()) VLOG(STR("[MoriaCppMod] [STAB] Audit cleared ({} locations)\n"), m_auditLocations.size());
...
void runStabilityAudit()
{
    ...
    for (int32_t i = 0; i < arrNum; i++)
    {
        ...
        if (state == STAB_DECONSTRUCTED || state == STAB_UNINITIALIZED || state == STAB_INITIALIZING) continue;
    ...
    for (auto& p : problems)
    {
        ...
        if (stabilityVfx && pc) spawnVfxAtLocation(pc, stabilityVfx, p.x, p.y, p.z);

        m_auditLocations.push_back({p.x, p.y, p.z, isCritical});

        if (pc) spawnPointLightAtLocation(pc, p.x, p.y, p.z, isCritical);
    }
