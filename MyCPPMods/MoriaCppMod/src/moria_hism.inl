        ...
        for (auto* prop : transFunc->ForEachProperty())
        {
            ...
            if (n == L"InstanceIndex" && off != 0)
                ...
                VLOG(STR("[MoriaCppMod] WARN: GetInstanceTransform bWorldSpace expected @{}, got @{}\n"),
                     (int)offsetof(GetInstanceTransform_Params, bWorldSpace),
                     off);
                ...
                VLOG(STR("[MoriaCppMod] WARN: GetInstanceTransform ReturnValue expected @{}, got @{}\n"),
                     (int)offsetof(GetInstanceTransform_Params, ReturnValue),
                     off);
        }
        VLOG(STR("[MoriaCppMod] Validated GetInstanceTransform_Params ({}B struct vs {}B UFunction)\n"),
             (int)sizeof(GetInstanceTransform_Params),
             transFunc->GetParmsSize());
    }
...
void findWorldLayout()
{
    ...
    findAllOfSafe(STR("WorldLayout"), objs);
    UObject* prev = m_worldLayout;
    if (!objs.empty()) m_worldLayout = objs[0];
    if (m_worldLayout && m_worldLayout != prev) VLOG(STR("[MoriaCppMod] [Bubble] WorldLayout found: {}\n"), (void*)m_worldLayout);
}
...
bool updateCurrentBubble()
{
    ...
    if (!isObjectAlive(m_worldLayout))
    {
        m_worldLayout = nullptr;
        return false;
    }
    if (safeClassName(m_worldLayout).empty())
    {
        m_worldLayout = nullptr;
        return false;
    }                                                                        // [rc.118] SEH probe
...
bool computeBubbleLocal(UObject* bubble, float wx, float wy, float wz, float& outLx, float& outLy, float& outLz)
{
    outLx = wx;
    outLy = wy;
    outLz = wz;
...
void captureBubbleInfo()
{
    ...
    if (!bdPath.empty()) clip += L"Definition Path: " + bdPath + L"\r\n";
    ...
    std::wstring pathField = !bdPath.empty() ? bdPath : (!bdName.empty() ? bdName : pathName);
...
void startReplay()
{
    ...
    findAllOfSafe(STR("GlobalHierarchicalInstancedStaticMeshComponent"), rawComps);
    if (rawComps.empty()) findAllOfSafe(STR("HierarchicalInstancedStaticMeshComponent"), rawComps);

    m_replay.compQueue.reserve(rawComps.size());
    for (auto* c : rawComps)
        m_replay.compQueue.emplace_back(c);
        ...
        while (m_replay.instanceIdx < count)
        {
            ...
            else if (transFunc)
            {
                ...
                for (size_t si = 0; si < m_savedRemovals.size(); si++)
                {
                    ...
                    if (!m_savedRemovals[si].bubbleId.empty() && !m_currentBubbleId.empty() && m_savedRemovals[si].bubbleId != m_currentBubbleId) continue;
...
void checkForNewComponents()
{
    ...
    findAllOfSafe(STR("GlobalHierarchicalInstancedStaticMeshComponent"), comps);
...
void toggleFlyMode()
{
    ...
    // Required for dedicated servers where local SetMovementMode is overridden
    {
        ...
        if (pc)
        {
            ...
            UObject* cheatMgr = cmProp ? *reinterpret_cast<UObject**>(reinterpret_cast<uint8_t*>(pc) + cmProp->GetOffset_Internal()) : nullptr;
            ...
            if (cheatMgr)
            {
                auto* flyFn = cheatMgr->GetFunctionByNameInChain(m_flyMode ? STR("Fly") : STR("Walk"));
                if (flyFn)
                {
    ...
    // Set locally on the local player's pawn for immediate visual feedback
    {
        ...
        {
            if (!m_flyMode) setBoolProp(movComp, L"bCheatFlying", false);
            ...
            if (m_flyMode) setBoolProp(movComp, L"bCheatFlying", true);
    ...
    std::wstring dtRowName;
    {
        ...
        else
        {
            ...
            if (fOK)
            {
                ...
                dumpFile << L"DT_Constructions: " << names.size() << L" rows, Actor@0x" << std::hex << actorOff << std::dec << L"\n\n";
                dumpFile.flush();
            }
            ...
            for (size_t i = 0; i < names.size(); i++)
            {
                ...
                {
                    if (!classPath.empty() && classPath.find(rowAssetPath) != std::wstring::npos) isMatch = true;
                    if (!isMatch && !recipeRef.empty() && rowAssetPath.find(recipeRef) != std::wstring::npos) isMatch = true;
                }
                ...
                {
                    std::wstring dispName = m_dtConstructions.readFText(rowNameStr.c_str(), L"DisplayName");
                    dumpFile << (isMatch ? L">>> MATCH" : L"   ") << L" [" << i << L"] " << rowNameStr << L"  Display=\"" << dispName << L"\"  ActorPath=\""
                             << rowAssetPath << L"\"\n";
                    dumpFile.flush();
                }
